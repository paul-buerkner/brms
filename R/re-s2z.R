...
.re_s2z_build_plan <- function(bframe) {
  ...
  active_q <- sort(unique(unlist(
    lapply(blocks, `[[`, "block_active_q"), use.names = FALSE
  )))
  inactive_q <- setdiff(original_q, active_q)
  active_names <- qnames[active_q]
  inactive_names <- qnames[inactive_q]
  active_index <- match(original_q, active_q)
  inactive_index <- match(original_q, inactive_q)
  blocks <- lapply(blocks, function(block) {
    block$match_active <- match(block$match_q, active_q)
    block
  })
  nlist(
    p, px, qnames, original_q, fixef, fixef_q, center,
    active_q, inactive_q, active_names, inactive_names,
    active_index, inactive_index, ids, set_id = ids[1L], blocks
  )
}
...
# Compatibility view for the existing numerical kernels. Repeated population
# metadata and fixed-only flat placeholders are constructed only here, never
# stored on the frame or on the plan's blocks.
re_s2z_plan_infos <- function(plan) {
  if (is.null(plan)) {
    return(list())
  }
  specs <- NULL
  if (!is.null(plan$prior)) {
    specs <- rep(list(re_s2z_flat_prior()), length(plan$qnames))
  ...
  lapply(plan$blocks, function(block) {
    info <- list(
      ...
      p = plan$p, context = block$context,
      block_active_q = block$block_active_q,
      active_q = plan$active_q, inactive_q = plan$inactive_q,
      active_names = plan$active_names, inactive_names = plan$inactive_names,
      active_index = plan$active_index, match_active = block$match_active
    )
    if (!is.null(specs)) {
      info$prior <- specs
    }
...
# Describe all local blocks using the legacy numerical-kernel interface.
re_s2z_infos <- function(bframe, prior = NULL) {
  re_s2z_plan_infos(re_s2z_plan(bframe, prior = prior))
}
