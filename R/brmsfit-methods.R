...
#' @param ... Currently ignored
#' 
#' @return A list of arrays (one for each grouping level).
#' 
#' @examples
#' \dontrun{
#' fit <- brm(riders ~ temp + windspeed + ar(1, gr = workday, latent = TRUE),
...
ac_latent.brmsfit <- function(object, summary = TRUE, robust = FALSE,
                              probs = c(0.025, 0.975), ...) {
  contains_draws(object)
  acef <- tidy_acef(object$formula)
  has_latent <- parameterize_ac_effects(acef)
  has_time <- has_explicit_ac_time(acef)
  if (!has_latent) {
    stop2("Model does not have latent autocorrelated effects.")
  }
  ...
  if (gr == "NA") {
    ...
    if (summary) {
      if(has_time) {
        times <- with(sdata, ac_time_points)
        colnames(out[[1]]) <- times
      } else {
        colnames(out[[1]]) <- 1:ncol(out[[1]])
      }
    ...
    for (g in seq_len(N_tg)) {
      if (has_time) {
        err_range <- with(sdata, begin_err_gr[g]:end_err_gr[g])
        times <- with(sdata, ac_time_points[err_range])
      } else {
        err_range <- with(sdata, begin_tg[g]:end_tg[g])
        times <- 1:length(err_range)
      }
      out[[tg_levels[g]]] <- draws[,err_range]
      if (summary) {
        colnames(out[[tg_levels[g]]]) <- times
        out[[tg_levels[g]]] <- posterior_summary(out[[tg_levels[g]]],
