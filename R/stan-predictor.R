...
stan_fe <- function(bframe, prior, stanvars, threads, primitive,
  ...
  if (length(fixef) && !primitive) {
    ...
    if (sparse) {
      ...
      # Physical zero-sum deviations absorb their omitted means into theta.
      # Both likelihood paths must therefore use theta in place of public b.
      beta <- if (s2z) stan_re_s2z_coef(bframe) else glue("b{sfx_b}{p}")
      eta_fe <- glue(" + X{sfx_X}{p}{slice} * {beta}")
    }
...
stan_re <- function(bframe, prior, normalize, ...) {
  ...
  out <- list()
  # Omitted means from different grouping factors enter the same population
  # coefficients. Their population priors must be evaluated jointly once per
  # predictor, after all participating group blocks have been constructed.
...
.stan_re <- function(id, bframe, prior, threads, normalize,
  ...
  # Covariance priors and public outputs are shared by both parameterizations.
  # S2Z recovers conventional effects after sampling their omitted means.
  s2z <- isTRUE(r$s2z[1])
  r_def <- if (s2z) "gen_def" else "tpar_def"
  if (nrow(r) > 1L && r$cor[1]) {
    ...
    str_add(out[[r_def]]) <- glue(
      "  matrix[N_{id}, M_{id}] r_{id};  // actual group-level effects\n",
      "  // using vectors speeds up indexing in loops\n"
    )
  }
  r_comment <- str_if(nrow(r) == 1L || !r$cor[1],
                      "  // actual group-level effects")
  str_add(out[[r_def]]) <- cglue(
    "  vector[N_{id}] r_{idp}_{r$cn};{r_comment}\n"
  )
...
# Expand the structural plan for the Stan kernels. Fixed-only coordinates
# receive flat placeholders because their priors are generated by stan_fe().
stan_re_s2z_plan_infos <- function(plan) {
  if (is.null(plan)) {
    return(list())
  }
  specs <- NULL
  if (!is.null(plan$prior)) {
    specs <- rep(list(parse_re_s2z_prior("")), length(plan$qnames))
    specs[plan$active_q] <- unname(plan$prior)
  }
  lapply(plan$blocks, function(block) {
    info <- list(
      id = block$id, r = block$r, qnames = plan$qnames,
      match_q = block$match_q, center = plan$center, fixef = plan$fixef,
      p = plan$p, active_q = plan$active_q
    )
    if (!is.null(specs)) {
      info$prior <- specs
    }
...
stan_re_s2z_systems <- function(bframe, prior) {
  lapply(Filter(has_re_s2z, all_bframel(bframe)), function(bfl) {
    ...
    infos <- stan_re_s2z_plan_infos(plan)
    system <- list(set_id = plan$set_id, ids = plan$ids, infos = infos)
...
# The caller emits shared group data, covariance priors, and public outputs.
...
stan_re_s2z_public_re_comp <- function(r, scalar_mean = FALSE, indent = "  ",
  ...
  cglue("{indent}{r_public} = {rs2z} + mean_r_s2z_{id}{mean_index};\n")
}

# Map the recovered population coefficients back through design centering.
...
stan_fe_s2z <- function(bframe, prior, normalize) {
  ...
  # The likelihood uses theta; conventional b coefficients are recovered
  # jointly with the omitted group means in generated quantities.
  if (plan$center) {
    str_add(out$gen_def) <- glue(
      "  real Intercept{p};\n",
      str_if(length(fixef), glue("  vector[Kc{p}] b{p};\n")),
      "  real b{p}_Intercept;\n"
    )
  } else {
    str_add(out$gen_def) <- glue("  vector[{K_s2z}] b{p};\n")
  }
  ...
  if (length(inactive_s2z)) {
    # With center = FALSE, an unmatched intercept is an ordinary class-b
    # coefficient and uses the same fixed-only prior path as other columns.
...
.stan_re_s2z_explicit_block <- function(id, set, prior, normalize,
  ...
  str_add(out$tpar_comp) <- glue(
    "  for (k in 1:M_{id}) {{\n",
    "    rs2z_{id}[, k] = sum_to_zero_constrain_brms(",
  ...
  str_add(out$tpar_comp) <- glue(
    ...
    "mdivide_left_tri_low_brms(L_Sigma_s2z_{id}, rs2z_{id}');\n",
    "{explicit_group_quad_code}",
    "  }}\n",
    cglue("  {rs2z} = rs2z_{id}[, {J}];\n")
  )
  str_add(out$pll_args) <- cglue(", vector {rs2z}")
...
.stan_re_s2z_explicit_system <- function(set, normalize, out = list()) {
  ...
  for (block in infos) {
    str_add(out$gen_comp) <- stan_re_s2z_public_re_comp(block$r)
  }
  ...
  if (use_matheron && is_cor) {
    ...
    group_score_code <- if (is_student) {
      glue(
        ...
        "    group_quad_s2z_{id} = 0.0;\n",
        "    for (k in 1:M_{id}) {{\n",
        "      vector[N_{id}] white_group_s2z = rs2z_{id}[, k] / ",
        "{scale}[k];\n",
...
.stan_re_s2z_joint_matheron <- function(set, prior, threads, normalize, ...) {
  ...
  for (block in infos) {
    str_add(out$gen_def) <- glue(
      "  vector[M_{block$id}] mean_r_s2z_{block$id};\n"
    )
  }
...
.stan_re_s2z <- function(id, system, bframe, prior, threads, normalize,
  ...
  str_add(out$tpar_def) <- glue(
    ...
    "  real<lower=0> group_quad_s2z_{id};\n",
    "  // using vectors speeds up indexing in loops\n",
    cglue("  vector[N_{id}] rs2z_{idp}_{r$cn};\n")
  )
...
.stan_re_s2z_independent <- function(id, r, info, normalize, out = list()) {
  ...
  for (j in seq_len(M)) {
    str_add(out$tpar_comp) <- glue(
      "  {rs2z[j]} = sum_to_zero_constrain_brms(",
  ...
  for (j in seq_len(M)) {
    contrast_score <- if (is_student) {
      glue(" - dot_product({rs2z[j]}, group_prec_s2z_{id})")
    } else {
      ""
    }
    str_add(out$tpar_comp) <- glue(
  ...
  for (j in seq_len(M)) {
    standardized <- glue(
      "({rs2z[j]} + mhat_s2z_{id}[{j}]) / sd_{id}[{j}]",
...
.stan_re_s2z_scalar <- function(id, r, info, normalize, out = list()) {
  ...
  str_add(out$tpar_comp) <- glue(
    "  {rs2z} = sum_to_zero_constrain_brms(z_s2z_{id});\n"
  )
  ...
  # near zero during warmup. For Gaussian effects, sum(rs2z) is exactly zero
  ...
  if (is_student) {
    ...
    str_add(out$tpar_comp) <- glue(
      ...
      "dot_product({rs2z}, group_prec_s2z_{id})) / D_s2z_{id};\n",
      "  }}\n"
    )
  } else {
    str_add(out$tpar_comp) <- glue(
  ...
  str_add(out$tpar_comp) <- glue(
    ...
    "mhat_s2z_{id};\n",
    "  {{\n",
    "    vector[N_{id}] white_s2z = ({rs2z} + mhat_s2z_{id}) / ",
    "sd_{id}[1]",
    str_if(is_student, glue(" ./ group_scale_s2z_{id}")),
    ";\n",
  ...
  str_add(out$pll_args) <- glue(", vector {rs2z}")
...
stan_eta_re <- function(bframe, threads) {
  ...
  for (id in unique(reframe$id)) {
    ...
    idp <- paste0(r$id, usc(combine_prefix(rpx)))
    idresp <- paste0(r$id, usc(rpx$resp))
    rprefix <- str_if(isTRUE(r$s2z[1]), "rs2z_", "r_")
    if (r$gtype[1] == "mm") {
      ng <- seq_along(r$gcall[[1]]$groups)
      for (i in seq_rows(r)) {
