...
choose_N <- function(draws) {
  ...
  .choose_N <- function(draws) {
    if (!is.null(draws$data$N_tg)) draws$data$N_tg else draws$data$N
  }
  if (is.mvbrmsdraws(draws)) {
    N <- unique(ulapply(draws$resps, .choose_N))
    if (length(N) > 1L) {
      stop2("'log_lik' is currently not supported for this model.")
    }
  } else {
    N <- .choose_N(draws)
  }
