extract_effects <- function(x, family = NA, autocor = NULL, 
  ...
  if (isTRUE(x[["nl"]])) {
    ...
    y$nlpars <- extract_nonlinear(pforms[nlpars], y$fixed)
    re_terms <- NULL
  } else {
    if (length(nlpars)) {
  ...
  y$random <- extract_random(re_terms)
  # evaluate autocor formula
  y$time <- extract_time(autocor$formula)
