...
This vignette is about monotonic effects, a special way of handling discrete predictors that are at least on an ordinal scale. The idea was brought to me by   Emmanuel Charpentier, who deserves full credit for it. A predictor, which we want to model as monotonic, must either be integer valued or an ordered factor. As opposed to a continuous predictor, predictor categories (or integers) are not assumend to be equidistant with respect to their effect on the response variable. Instead, the distance between adjacent predictor categories (or integers) is estimated from the data and may vary across categories. This is realized by parameterizing as follows: One parameter, $b$, takes care of the direction and size of the effect similar to an ordinary regression parameter, while an additional parameter vector, $\zeta$, estimates the normalized distances between consecutive predictor categories. For a single monotonic predictor, $x$, the linear predictor term of observation $n$ looks as follows:

$$\eta_n = b \sum_{i = 1}^{x_n} \zeta_i$$

The parameter $b$ can take on any real value, while $\zeta$ is a simplex, which means that is it satisfies $\zeta_i \in [0,1]$ and $\sum_{i = 1}^K \zeta_i = 1$ with $K$ being the number of categories (or highest integer in the data).

## A Simple Monotonic Model

A main application of monotonic effects are ordinal predictors that can this way be modeled without falsely treating them either as continuous or as unordered categorical predictors. As an example, suppose we are interested in the relationship of yearly income (in $) and life satisfaction measured on an arbitrary scale from 0 to 100. Usually, people are not asked for the exact income, since they may hesitate to answer. Instead, they are asked to rank themselves in one of certain classes, say: 'below 20k', 'between 20k and 40k', 'between 40k and 100k' and 'above 100k'. We use some simulated data for illustration purposes.
...
```
For technical reasons, monotonic group-level effects have to be specified in separate terms in the model formula. Further, we have used the abbrevation `mono` for `monotonic`, which helps in shortening the formula a bit. The summary output
