...
Assume, we ignore the fact that the phenotype is count data and fit a linear normal model instead.
...
Apparently, the distribution of the phenotype prediced by the Poisson model resembles the original distribution of the phenotype pretty closely, but the normal models fails to do so. We can also apply leave-one-out cross-validation for direct numerical comparison of model fit.

```{r}
LOO(model_pois, model_normal)
```

Since smaller values of LOO indicate better fit, it is again evident that the poisson model fits the data better than the normal model.
