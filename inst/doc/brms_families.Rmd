...
The density of the **cumulative** family is giveny by
$$f(y_i) = g(\tau_{y_i + 1} - \eta_i) - g(\tau_{y_i} - \eta_i)$$ 
The densities of the **sratio** and **cratio** families are given by
$$f(y_i) =  g(\tau_{y_i + 1} - \eta_i) \prod_{k = 1}^{y_i} (1 - g(\tau_{k} - \eta_i))$$
and 
$$f(y_i) = (1 - g(\eta_i - \tau_{y_i + 1})) \prod_{k = 1}^{y_i} g(\eta_i - \tau_{k})$$
respectively. Note that both families are equivalent for symmetric link functions such as logit or probit. The density of the **acat** family is given by
...
Currently implemented families are **hurdle_poisson**, **hurdle_negbinomial**, and **hurdle_gamma**.  