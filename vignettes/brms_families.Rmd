...
$$
$\Gamma$ denotes the gamma function and $\nu > 1$ are the degrees of freedom. As $\nu \rightarrow \infty$, the student distribution becomes the gaussian distribution. The densitiy of the *skew_normal* family is given by
$$
f(y_i) = \frac{1}{\sqrt{2\pi}\sigma} \exp\left(-\frac{1}{2}\left(\frac{y_i - \mu_i}{\sigma}\right)^2\right) \left(1 + \text{erf} \left( \alpha \left(\frac{y_i - \mu_i}{\sigma \sqrt{2}} \right) \right) \right)
$$
where $\sigma$ is the positive scale parameter, $\alpha$ the skewness parameter and $\text{erf}$ denotes the error function of the gaussian distribution. If $\alpha = 0$, the skew-normal distribution becomes the gaussian distribution. For location shift models, $y_i$ can be any real value. 
