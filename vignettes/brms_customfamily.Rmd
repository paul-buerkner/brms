...
%\VignetteIndexEntry{Define custom response distributins with brms}
...
The **brms** package comes with a lot of built-in response distributions -- usually called *families* in R -- to specify, among others, linear, count data, survival, response times, or ordinal models (see `help(brmsfamily)` for an overview). Despite supporting over two dozens families, there is still a long list of distributions, which are not natively supported. The present vignette will explain how to specify such *custom* families in **brms**. By doing that, users can benefit from the enormous modeling flexibility and post-processing options of **brms** even when using self-defined response distributions. 
...
where $p$ is the success probability. In the classical binomial model, we will directly predict $p$ on the logit-scale, which means that for each observation $i$ we compute the success probability $p_i$ as
...
A drawback of the binomial model is that -- after taking into account the linear predictor -- its variance is fixed to $\text{Var}(y_i) = T p_i (1 - p_i)$. All variance exceeding this value cannot be not taken into account by the model. There are multiple ways of dealing with this so called *overdispersion* and the solution described below will serve as an illustrative example of how to define custom families in **brms**. 
...
The `predict` function looks pretty similar to the corresponding `log_lik` function, except that we are now creating random samples of the response instead of log-liklihood values. Again, we are using an exposed **Stan** function for convenience. With `predict` to be working, we can engage for instance in posterior-predictive checking:

```{r pp_check}
pp_check(fit2)
```

When defining the `fitted` function, we have to keep in mind that it has only a `draws` argument and should compute the mean response values for all observations at once. Since the mean of the beta-binomial distribution is $\text{E}(y) = \mu N$ definition of the corresponding `fitted` function is not too complicated, but we need to get the dimension of parameters and data in line.
...
## Turning a Custom into a Native Family

Family functions built natively into **brms** are saver to use and more convenient, as they require much less user input. The current internal structure of **brms** makes it rather hard for an outside developer to implement a native family and submit it as a pull request to [GitHub](https://github.com/paul-buerkner/brms). If you feel that your custom family is general enough to be useful to other users, please feel free to open an issue on [GitHub](https://github.com/paul-buerkner/brms/issues), in which you provide general information about your family as well as all the necessary functions described above. Provided that you convince me, I will then go ahead and make the required changes to the **brms** code base.
