...
- Do not use hyper-threading as a default.
...
likelihood which does is by far less expensive to evaluate
computationally. Morever, this example is chosen in order to
...
the chunk size and order of partial sums will be the same between
calls.
...
increase successivley the number of chunks a given data set is split
into with the static scheduler and run this on a single core. This way
one can control the number of *partial sum* accuratley and monitor the
execution time as it increases. These experiements are run with only a
...
The vertical dashed line marks the physical number of CPU cores on the
machine this was run. We can see that there is no further reduction in
execution time when increasing the thread count to be greater than the
number of pyhsical CPUs. Hence, the use of hyperthreading is not
helpful when aiming to maximize the speed of a Stan program. Moreover,
we can see that for 2 - 4 CPU cores all the `grainsize`s perform
similarily well. The overall speedups may not seem impressive in this
case, which is attributed in this case to the large number of random
effects. However, we still obtain a doubling in speed when using 4
cores and on 2 cores and increase of 40% is reached.
...
have, but it occurs in practice), then one may consider the use of
hyper-threading. Doing so will share the ressources evenly accross all
chains and whenever the fastest chain finishes, the freed ressources
