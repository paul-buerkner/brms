...
test_that("small-matrix helpers cover group-effect solves without duplicate definitions", {
  Omega <- diag(nlevels(s2z_opt_dat$g))
  dimnames(Omega) <- rep(list(levels(s2z_opt_dat$g)), 2L)
  forms <- list(
    ordinary = bf(
      y ~ x + (1 + x | gr(g, center = 0.4)),
      sigma ~ x + (1 + x | gr(h, center = 0.4))
    ),
    s2z = y ~ x + (1 + x | gr(g, s2z = TRUE, center = "auto")),
    scalar_cov = y ~ 1 + (1 | gr(g, s2z = TRUE, cov = Omega)),
    joint = y ~ x + (1 + x | gr(g, s2z = TRUE)) +
      (1 + x | gr(h, s2z = TRUE)),
    cross = bf(mvbind(y, z) ~ 1 + (1 | p | gr(g, s2z = TRUE))) +
      set_rescor(FALSE),
    larger = y ~ x + z + (1 + x + z | gr(g, s2z = TRUE))
  )
  for (form in forms) {
    code <- stancode(form, data = s2z_opt_dat, data2 = list(Omega = Omega))
    expect_equal(
    ...
    expect_false(grepl(
      "(mdivide_left_tri_low|mdivide_right_tri_low|cholesky_decompose)\\(",
      body
    ))
  }
...
test_that("fixed S2Z maps live in transformed data", {
  ...
  expect_match2(code, "vector[M_1] whitened_h_s2z;")
  expect_false(grepl(
