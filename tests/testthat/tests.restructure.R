...
test_that("rename_prior returns expected lists", {
  pars <- c(
    "b", "b_1", "bp", "bp_1", "prior_b", "prior_b__1",
...
test_that("rename_old_re and rename_old_re2 return expected lists", {
  ...
  pars <- c(
    "b_a_Intercept", "b_a_x", "sd_a_g_Intercept", "sd_a_g_x",
    "cor_a_g_Intercept_x", paste0("r_a_g[", 1:10, ",Intercept]"),
    paste0("r_a_g[", 1:10, ",x]")
  )
  dims <- list(
    "sd_a_g_Intercept" = numeric(0), "sd_a_g_x" = numeric(0),
  ...
  target <- list(
    ...
    list(
      pos = c(rep(FALSE, 5), rep(TRUE, 20)), oldname = "r_g_a",
      pnames = "r_g__a",
      fnames = c(
        paste0("r_g__a[", 1:10, ",Intercept]"),
  ...
  pars <- c(
    "b_a_Intercept", "b_a_x", "sd_g_a_Intercept", "sd_g_a_x",
    "cor_g_a_Intercept_a_x", paste0("r_g_a[", 1:10, ",Intercept]"),
    paste0("r_g_a[", 1:10, ",x]")
  )
  dims <- list(
    "sd_g_a_Intercept" = numeric(0), "sd_g_a_x" = numeric(0),
...
test_that("rename_old_sm return expected lists", {
  target <- list(
    ...
    list(
      pos = c(FALSE, FALSE, rep(TRUE, 6), rep(FALSE, 9)),
  ...
  pars <- c(
    "sds_sigma_t2x0", "sds_sx1kEQ9",
    paste0("s_sigma_t2x0[", 1:6, "]"),
    paste0("s_sx1kEQ9[", 1:9, "]")
  )
  dims <- list(
    sds_sigma_t2x0 = numeric(0), sds_sx1kEQ9 = numeric(0),
...
test_that("rename_old_categorical works correctly", {
  ...
  pars <- c(
    "b_cat2_Intercept", "b_cat3_Intercept",
    "b_cat2_x", "b_cat3_x"
  )
