...
test_that("all S3 methods have reasonable ouputs", {
  ...
  expect_equal(dimnames(fixef), list(c("Intercept", "Trt_c"),
                                     c("mean", "sd")))
  # formula
  expect_equal(formula(fit), count ~ Trt_c + offset(log_Age_c) + (1+Trt_c|visit))
  # hypothesis
  ...
  expect_error(hypothesis(fit, "Intercept > x"), fixed = TRUE,
               "The following parameters cannot be found in the model: b_x")
  # omit launch_shiny
  # logLik
  ...
  expect_true("r_visit" %in% up$exclude)
  expect_error(update(fit, family = "gaussian"),
               "family cannot be updated")
  expect_error(updata(fit, data = new_data),
