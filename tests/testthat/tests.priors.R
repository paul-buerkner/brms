...
test_that("prior tags are correctly applied", {
  ...
  prior5 <- prior(lkj_corr_cholesky(3), class = "L")
  ...
  expect_equal(v[which(v$tag == "prior_tag1"),], "prior_tag1")
  expect_equal(v$tag[which(v$prior == prior2$prior)], "prior_tag2")
  expect_equal(v$tag[which(v$prior == prior3$prior)], "prior_tag3")
  expect_equal(v$tag[which(v$prior == prior4$prior)], "prior_tag4")
