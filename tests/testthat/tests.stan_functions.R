...
test_that("self-defined Stan functions work correctly", {
  ...
  cov_ar1_R <- get_cov_matrix_ar1(
    ar = matrix(0.5), sigma = 2,
    se2 = 0, nrows = 3
  )[1, , ]
  expect_equal(cov_matrix_ar1(0.5, 2, 3), cov_ar1_R)
  cov_ma1_R <- matrix(get_cov_matrix_ma1(
    ma = matrix(-0.3), sigma = 3,
    se2 = 0, nrows = 1
  )[1, , ])
  expect_equal(cov_matrix_ma1(-0.3, 3, 1), cov_ma1_R)
  cov_arma1_R <- get_cov_matrix_arma1(
    ar = matrix(-0.5), ma = matrix(0.7),
    sigma = 4, se2 = 0, nrows = 5
  )[1, , ]
  ...
  expect_equal(
    inv_gaussian_lpdf(y, mu, shape, log(y), sqrt(y)),
    dinvgauss(y, mean = mu, shape = shape, log = TRUE)
  )
  expect_equal(
    inv_gaussian_lcdf(y, mu, shape, log(y), sqrt(y)),
    pinvgauss(y, mean = mu, shape = shape, log = TRUE)
  )
  expect_equal(
    inv_gaussian_lccdf(y, mu, shape, log(y), sqrt(y)),
  ...
  expect_equal(
    inv_gaussian_vector_lpdf(y, mu, shape, sum(log(y)), sqrt(y)),
  ...
  expect_equal(
    exgaussian_lpdf(y, mu, sigma, beta),
    dexgaussian(y, mu, sigma, beta, log = TRUE)
  )
  expect_equal(
    exgaussian_lcdf(y, mu, sigma, beta),
    pexgaussian(y, mu, sigma, beta, log = TRUE)
  )
  expect_equal(
    exgaussian_lccdf(y, mu, sigma, beta),
  ...
  expect_equal(
    wiener_diffusion_lpdf(y$q, y$resp, alpha, tau, beta, delta),
  ...
  for (i in seq_along(draws$data$Y)) {
    eta_zi_args <- list(
      y = draws$data$Y[i], eta = draws$eta[i],
    ...
    expect_equal(
      do.call(zero_inflated_poisson_lpmf, zi_args),
      loglik_zero_inflated_poisson(i, draws)
    )
    expect_equal(
      do.call(zero_inflated_poisson_logit_lpmf, eta_zi_args),
      loglik_zero_inflated_poisson(i, draws)
    )

    expect_equal(
      do.call(
        zero_inflated_neg_binomial_lpmf,
    ...
    expect_equal(
      do.call(
        zero_inflated_neg_binomial_logit_lpmf,
    ...
    expect_equal(
      do.call(hurdle_poisson_lpmf, hu_args),
      loglik_hurdle_poisson(i, draws)
    )
    expect_equal(
      do.call(hurdle_poisson_logit_lpmf, eta_hu_args),
      loglik_hurdle_poisson(i, draws)
    )

    expect_equal(
      do.call(
        hurdle_neg_binomial_lpmf,
    ...
    expect_equal(
      do.call(
        hurdle_neg_binomial_logit_lpmf,
    ...
    expect_equal(
      do.call(
        zero_inflated_binomial_logit_lpmf,
    ...
    draws2$f$link <- "logit"
    eta_zi_args <- list(
      y = draws2$data$Y[i], eta = draws$eta[i],
      eta_zi = draws$eta[i + 2]
    )
    zi_args <- list(
      y = draws2$data$Y[i], eta = draws$eta[i],
  ...
  draws$f$link <- "probit"
  expect_equal(
    cumulative_lpmf(draws$data$Y, eta, thres),
    loglik_cumulative(1, draws)[1]
  )
  draws$f$link <- "logit"
  expect_equal(
    sratio_lpmf(draws$data$Y, eta, thres),
  ...
  expect_equal(
    cratio_lpmf(draws$data$Y, eta, etap[1, , ], thres),
    loglik_cratio(1, draws)[1]
  )
  draws$f$link <- "cauchit"
  expect_equal(
    acat_lpmf(draws$data$Y, eta, etap[1, , ], thres),
  ...
  sd <- c(2, 7)
  expect_equal(
    t(chol(base::kronecker(A, diag(sd) %*% B %*% diag(sd)))),
