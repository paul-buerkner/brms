...
test_that("monolithic code retains later S2Z APIs and state", {
  public_arguments <- names(formals(gr))
  expect_true(all(c("center", "scale") %in% public_arguments))
  expect_silent(gr(g, s2z = TRUE, center = "auto"))
  expect_silent(gr(g, s2z = TRUE, scale = "varying"))
  ...
  for (term in c("rho_s2z", "sdlog", "sd_level", "z_sd_s2z")) {
