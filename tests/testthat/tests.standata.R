...
test_that(paste(
  "standata returns correct data names ",
  "for fixed and random effects"
), {
  expect_equal(
    sort(names(standata(rating ~ treat + period + carry
  ...
  expect_equal(
    ...
    sort(c(
      "N", "Y", "ncat", "K_mu2", "Kc_mu2", "X_mu2", "Z_1_mu2_1",
  ...
  expect_equal(
    ...
    sort(c(
      "N", "Y", "K", "Kc", "X", "Z_1_1", "Z_1_2", "J_1", "N_1", "M_1",
      "NC_1", "prior_only"
    ))
  )
  ...
  expect_equal(
    standata(y ~ x + (1 + x | g + h), dat),
    standata(y ~ x + (1 + x | g) + (1 + x | h), dat)
  )
})

test_that(paste(
  "standata handles variables used as fixed effects",
...
test_that("standata returns correct data names for addition terms", {
  dat <- data.frame(
    y = 1:10, w = 1:10, t = 1:10, x = rep(0, 10),
    c = sample(-1:1, 10, TRUE)
  )
  expect_equal(
    names(standata(y | se(w) ~ x, dat, gaussian())),
    c("N", "Y", "se", "K", "Kc", "X", "sigma", "prior_only")
  )
  expect_equal(
    names(standata(y | weights(w) ~ x, dat, "gaussian")),
    c("N", "Y", "weights", "K", "Kc", "X", "prior_only")
  )
  expect_equal(
    names(standata(y | cens(c) ~ x, dat, "student")),
    c("N", "Y", "cens", "K", "Kc", "X", "prior_only")
  )
  expect_equal(
    names(standata(y | trials(t) ~ x, dat, "binomial")),
    c("N", "Y", "trials", "K", "Kc", "X", "prior_only")
  )
  expect_equal(
    names(standata(y | trials(10) ~ x, dat, "binomial")),
    c("N", "Y", "trials", "K", "Kc", "X", "prior_only")
  )
  expect_equal(
    names(standata(y | thres(11) ~ x, dat, "acat")),
    c("N", "Y", "nthres", "K", "Kc", "X", "disc", "prior_only")
  )
  expect_equal(
    names(standata(y | thres(10) ~ x, dat, cumulative())),
    c("N", "Y", "nthres", "K", "Kc", "X", "disc", "prior_only")
  )
...
test_that(paste(
  "standata accepts correct response variables",
  "depending on the family"
), {
  expect_equal(standata(y ~ 1,
    data = data.frame(y = seq(-9.9, 0, 0.1)),
    family = "student"
  )$Y, as.array(seq(-9.9, 0, 0.1)))
  expect_equal(standata(y | trials(10) ~ 1,
    data = data.frame(y = 1:10),
    family = "binomial"
  )$Y, as.array(1:10))
  expect_equal(standata(y ~ 1,
    data = data.frame(y = 10:20),
    family = "poisson"
  )$Y, as.array(10:20))
  expect_equal(standata(y ~ 1,
    data = data.frame(y = rep(-c(1:2), 5)),
    family = "bernoulli"
  )$Y, as.array(rep(1:0, 5)))
  expect_equal(standata(y ~ 1,
    data = data.frame(y = rep(c(TRUE, FALSE), 5)),
    family = "bernoulli"
  )$Y, as.array(rep(1:0, 5)))
  expect_equal(standata(y ~ 1,
  ...
  expect_equal(standata(y ~ 1,
    data = data.frame(y = rep(0, 5)),
    family = "bernoulli"
  )$Y, as.array(rep(0, 5)))
  expect_equal(standata(y ~ 1,
    data = data.frame(y = rep(1:10, 5)),
    family = "categorical"
  )$Y, as.array(rep(1:10, 5)))
  expect_equal(standata(y ~ 1,
    data = data.frame(y = rep(11:20, 5)),
    family = "categorical"
  )$Y, as.array(rep(1:10, 5)))
  expect_equal(standata(y ~ 1,
    data = data.frame(y = factor(rep(11:20, 5))),
    family = "categorical"
  )$Y, as.array(rep(1:10, 5)))
  expect_equal(standata(y ~ 1,
    data = data.frame(y = rep(1:10, 5)),
  ...
  expect_equal(
    standata(y ~ 1, data = dat, family = "acat")$Y,
    as.array(rep(1:10, 5))
  )
  expect_equal(standata(y ~ 1,
    data = data.frame(y = seq(1, 10, 0.1)),
    family = "exponential"
  )$Y, as.array(seq(1, 10, 0.1)))
...
test_that(paste(
  "standata rejects incorrect response variables",
  "depending on the family"
), {
  expect_error(
    standata(y ~ 1,
      data = data.frame(y = factor(1:10)),
      family = "student"
    ),
  ...
  expect_error(
    standata(y ~ 1,
      data = data.frame(y = rep(0.5:7.5), 2),
      family = "sratio"
    ),
  ...
  expect_error(
    standata(y ~ 1,
      data = data.frame(y = rep(-7.5:7.5), 2),
      family = "gamma"
    ),
...
test_that("standata suggests using family bernoulli if appropriate", {
  expect_message(
    standata(y | trials(1) ~ 1,
      data = list(y = rep(0:1, 5)),
      family = "binomial"
    ),
...
test_that("standata returns correct values for addition terms", {
  dat <- data.frame(
    y = rnorm(9), s = 1:9, w = 1:9, c1 = rep(-1:1, 3),
  ...
  expect_equivalent(
    standata(y | se(s) ~ 1, data = dat)$se,
    as.array(1:9)
  )
  expect_equal(
    standata(y | weights(w) ~ 1, data = dat)$weights,
    as.array(1:9)
  )
  expect_equal(
    standata(y | cens(c1) ~ 1, data = dat)$cens,
    as.array(rep(-1:1, 3))
  )
  expect_equal(
    standata(y | cens(c2) ~ 1, data = dat)$cens,
    as.array(rep(-1:1, 3))
  )
  expect_equal(
    standata(y | cens(c3) ~ 1, data = dat)$cens,
    as.array(c(rep(1:0, 4), 0))
  )
  expect_equal(
    standata(y | cens(c4, y + 2) ~ 1, data = dat)$rcens,
    as.array(c(rep(0, 5), dat$y[6:9] + 2))
  )
  expect_equal(
    standata(s | trials(10) ~ 1, dat,
      family = "binomial"
    )$trials,
    as.array(rep(10, 9))
  )
  expect_equal(
    standata(s | trials(t) ~ 1,
      data = dat,
      family = "binomial"
    )$trials,
    as.array(11:19)
...
test_that("standata handles multivariate models", {
  ...
  expect_error(
    standata(bf(mvbind(y1, y2, y2) ~ x) + set_resor(FALSE),
      data = dat
    ),
...
test_that("standata returns correct data for UNSTR covariance terms", {
  dat <- data.frame(
    y = 1:12, x = rnorm(12), tim = c(5:1, 1:5, c(0, 4)),
...
test_that("standata handles covariance matrices correctly", {
  ...
  B <- diag(1, 4)
  expect_error(
    standata(count ~ Trt + (1 | gr(visit, cov = B)),
      data = epilepsy, data2 = list(B = B)
    ),
  ...
  expect_warning(
    sdata <- standata(count ~ Trt + (1 | visit),
      data = epilepsy,
...
test_that("standata correctly prepares data for monotonic effects", {
  ...
  prior <- c(
    prior(dirichlet(c(1, 0.5, 2)), simo, coef = "v"),
...
test_that("standata returns correct group ID data", {
  form <- bf(
    count ~ Trt + (1 + Trt | 3 | visit) + (1 | patient),
  ...
  form <- bf(count ~ a, sigma ~ (1 | 3 | visit) + (Trt || patient),
    a ~ Trt + (1 + Trt | 3 | visit) + (1 | patient),
    nl = TRUE
  )
  sdata <- standata(form, data = epilepsy, family = student())
  expect_true(all(c(
    "Z_1_sigma_1", "Z_2_a_3", "Z_2_sigma_1",
    "Z_3_a_1"
  ) %in% names(sdata)))
})
...
test_that("standata handles category specific effects", {
  ...
  expect_error(
    standata(rating ~ 1 + (treat + cs(1) | subject),
      data = inhaler, family = "cratio"
    ),
...
test_that("standata handles wiener diffusion models", {
  ...
  expect_equal(sdata$dec, as.array(dat$resp))
  expect_error(
    standata(q | dec(test) ~ x, data = dat, family = wiener()),
...
test_that("standata handles multi-membership models", {
  ...
  expect_error(
    ...
    "Invalid term 'mmc(w1, w2, y)':",
    fixed = TRUE
  )
  expect_error(
    standata(y ~ (mmc(w1, w2) * y | mm(g1, g2)), data = dat),
    "The term 'mmc(w1, w2):y' is invalid",
    fixed = TRUE
  )
...
test_that("standata handles calls to the 'poly' function", {
  ...
  expect_equal(
    colnames(standata(y ~ 1 + poly(x, 3), dat)$X),
...
test_that("standata includes data for CAR models", {
  ...
  sdata <- standata(y ~ x + car(W, gr = group), dat,
    data2 = dat2,
  ...
  dat2$W[10, 1] <- 4
  expect_message(
    standata(y ~ car(W, gr = group), dat, data2 = dat2),
  ...
  dat2$W[1, 10] <- 0
  expect_error(
    standata(y ~ car(W), dat, data2 = dat2),
    "'M' for CAR terms must be symmetric"
  )
  dat2$W[10, 1] <- 0
  expect_error(
    SW(standata(y ~ x + car(W), dat, data2 = dat2)),
...
test_that("standata includes data of special priors", {
  ...
  # horseshoe prior
  hs <- horseshoe(7,
    scale_global = 2, df_global = 3,
    df_slab = 6, scale_slab = 3
  )
  sdata <- standata(y ~ x1 * x2,
    data = dat,
    prior = set_prior(hs)
  )
...
test_that("data for multinomial, dirichlet_multinomial and dirichlet models is correct", {
  ...
  expect_error(
    standata(t ~ x, data = dat, family = dirichlet()),
...
test_that("variables in data2 can be used in population-level effects", {
  ...
  sdata <- standata(y ~ foo(x1, x2, x3, idx = id),
    data = dat,
    data2 = list(id = c(3, 1))
  )
