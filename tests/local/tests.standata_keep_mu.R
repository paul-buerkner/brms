...
test_that(paste("standata returns correct data names ",
                "for fixed and random effects"), {
                  expect_equal(sort(names(standata(rating ~ treat + period + carry
                               ...
                               sort(c("N", "Y",  "K_mu", "Kc_mu", "X_mu", "Z_1_mu_1",
                  ...
                  expect_equal(sort(names(standata(rating ~ treat + period + carry
                               ...
                               sort(c("N", "Y", "K_mu", "Kc_mu", "X_mu", "Z_1_mu_1", "Z_1_mu_2", "J_1", "N_1", "M_1",
                                      "NC_1", "prior_only")))

                  dat <- data.frame(y = 1:10, g = 1:10, h = 11:10, x = rep(0,10))
                  expect_equal(sort(names(standata(y ~ 0 + Intercept + x + (1|g) + (1|h),
                                                   dat, "poisson"))),
                               sort(c("N", "Y", "K_mu", "X_mu", "Z_1_mu_1", "Z_2_mu_1",
                                      "J_1", "J_2", "N_1", "M_1", "NC_1", "N_2", "M_2", "NC_2",
                                      "prior_only")))
                  expect_true(all(c("Z_1_mu_1", "Z_1_mu_2", "Z_2_mu_1", "Z_2_mu_2") %in%
...
test_that(paste("standata handles variables used as fixed effects",
                "and grouping factors at the same time"), {
                  ...
                  expect_equal(colnames(standata$X_mu), c("Intercept", "xb", "xc"))
                  expect_equal(standata$J_1, as.array(rep(1:3, 3)))
                  standata2 <- standata(y ~ x + (1|x), data = data,
                                        control = list(not4stan = TRUE))
                  expect_equal(colnames(standata2$X_mu), c("Intercept", "xb", "xc"))
                })
...
test_that("standata handles multivariate models", {
  ...
  sdata_names <- c(
    "N", "J_1_y1",  "cens_y1", "Kma_mu_y1", "Z_1_mu_y2_3",
    "Zs_mu_y2_1_1", "Y_y2", "con_theta_x", "X_mu2_x"
  )
...
test_that("standata handles the 'subset' addition argument correctly", {
  ...
  expect_equal(nrow(sdata$X_mu_y2), nsub2)
})
...
test_that("standata returns correct data for ARMA terms", {
  ...
  expect_equal(sdata$begin_tg_mu, as.array(c(1, 6)))
  expect_equal(sdata$nobs_tg_mu, as.array(c(5, 5)))

  sdata <- standata(y ~ x + ar(tim), data = dat, family = poisson(),
                    prior = prior(horseshoe(), class = sderr, dpar = mu))
...
test_that("standata returns correct data for UNSTR covariance terms", {
  ...
  expect_equal(sdata$Jtime_tg_mu, Jtime)
})
...
test_that("standata correctly prepares data for monotonic effects", {
  ...
  sdata_names <- c("Xmo_mu_1", "Imo_mu", "Jmo_mu",  "con_simo_mu_8", "con_simo_mu_5")
  expect_true(all(sdata_names %in% names(sdata)))
  expect_equivalent(sdata$Xmo_mu_1, as.array(data$x1 - 1))
  expect_equivalent(sdata$Xmo_mu_2, as.array(as.numeric(data$x2) - 1))
  expect_equal(
    as.vector(unname(sdata$Jmo_mu)),
    rep(c(max(data$x1) - 1, length(unique(data$x2)) - 1), 4)
  )
  expect_equal(sdata$con_simo_mu_1, as.array(rep(1, 3)))
  ...
  prior <- c(
    set_prior("normal(0,1)", class = "b", coef = "mox1", dpar = 'mu'),
    set_prior("dirichlet(c(1, 0.5, 2))", class = "simo", coef = "mox11", dpar = 'mu'),
    prior_(~dirichlet(c(1, 0.5, 2)), class = "simo", coef = "mox1:mox21", dpar = 'mu')
  )
  sdata <- standata(y ~ mo(x1)*mo(x2), data = data, prior = prior)
  expect_equal(sdata$con_simo_mu_1, as.array(c(1, 0.5, 2)))
  expect_equal(sdata$con_simo_mu_3, as.array(c(1, 0.5, 2)))

  # test issue #924 (conditional monotonicity)
  prior <- c(prior(dirichlet(c(1, 0.5, 2)), simo, coef = "v", dpar = "mu"),
             prior(dirichlet(c(1,3)), simo, coef = "w", dpar = "mu"))
  sdata <- standata(y ~ y*mo(x1, id = "v")*mo(x2, id = "w"),
                    data, prior = prior)
  expect_equal(sdata$con_simo_mu_1, as.array(c(1, 0.5, 2)))
  expect_equal(sdata$con_simo_mu_2, as.array(c(1, 3)))
  expect_true(!"sdata$con_simo_mu_3" %in% names(sdata))
  ...
  prior <- c(set_prior("dirichlet(c(1,0.5,2))", class = "simo", coef = "mox21", dpar = 'mu'))
  expect_error(
...
test_that("standata returns data for GAMMs", {
  ...
  expect_equal(sdata$nb_mu_1, 1)
  expect_equal(as.vector(sdata$knots_mu_2), 8)
  expect_equal(dim(sdata$Zs_mu_1_1), c(10, 8))
  expect_equal(dim(sdata$Zs_mu_2_1), c(10, 8))
  ...
  expect_true(all(c("knots_mu_1", "knots_mu_2") %in% names(sdata)))
  ...
  expect_equal(sdata$nb_mu_1, 3)
  expect_equal(as.vector(sdata$knots_mu_1), c(9, 6, 6))
  expect_equal(dim(sdata$Zs_mu_1_1), c(10, 9))
  expect_equal(dim(sdata$Zs_mu_1_3), c(10, 6))
...
test_that("standata returns correct group ID data", {
  ...
  expect_true(all(c("Z_1_mu_1", "Z_2_mu_2", "Z_3_shape_1", "Z_2_shape_3") %in%
                    names(sdata)))
...
test_that("standata handles category specific effects", {
  ...
  expect_equivalent(sdata$Z_1_mu_3, as.array(rep(1, nrow(inhaler))))
  sdata <- standata(rating ~ period + carry + (cs(treat)|subject),
                    data = inhaler, family = cratio())
  expect_equivalent(sdata$Z_1_mu_4, as.array(inhaler$treat))
  expect_warning(
...
test_that("standata handles noise-free terms", {
  ...
  expect_equal(unname(sdata$Csp_mu_3), as.array(dat$x))
  expect_equal(sdata$Ksp_mu, 6)
...
test_that("standata handles multi-membership models", {
  ...
  expect_equal(unname(sdata$Z_1_mu_1_1), as.array(rep(1, 10)))
  expect_equal(unname(sdata$Z_1_mu_1_2), as.array(rep(1, 10)))
  ...
  expect_equal(unname(sdata$Z_1_mu_2_1), as.array(dat$w1))
  expect_equal(unname(sdata$Z_1_mu_2_2), as.array(dat$w2))
...
test_that("Cell-mean coding can be disabled", {
  ...
  expect_true(!"Z_1_mu_disc_2" %in% names(sdata))

  bform <- bf(y ~ 0 + g + (1 | y), cmc = FALSE)
  sdata <- standata(bform, df)
  expect_equal(sdata$X_mu, target)
  expect_equal(unname(sdata$Z_1_mu_1), as.array(rep(1, 10)))
})
...
test_that("standata correctly includes offsets", {
  ...
  expect_equal(sdata$offsets_mu, as.array(data$c))
  expect_equal(sdata$offsets_sigma, as.array(data$c + 1))
  sdata <- standata(y ~ x + offset(c) + offset(x), data)
  expect_equal(sdata$offsets_mu, as.array(data$c + data$x))
})
...
test_that("standata includes data for Gaussian processes", {
  ...
  expect_equal(max(sdata$Xgp_mu_1) - min(sdata$Xgp_mu_1), 1)
  sdata <- standata(y ~ gp(x1, scale = FALSE), dat)
  expect_equal(max(sdata$Xgp_mu_1) - min(sdata$Xgp_mu_1), max(dat$x1) - min(dat$x1))

  sdata <- SW(standata(y ~ gp(x1, by = z, gr = TRUE, scale = FALSE), dat))
  expect_equal(sdata$Igp_mu_1_2, as.array(4))
  expect_equal(sdata$Jgp_mu_1_4, as.array(1:5))
  expect_equal(sdata$Igp_mu_1_4, as.array(6:10))

  sdata <- SW(standata(y ~ gp(x1, by = y, gr = TRUE), dat))
  expect_equal(sdata$Cgp_mu_1, as.array(dat$y))
})
...
test_that("standata includes data for approximate Gaussian processes", {
  ...
  expect_equal(sdata$NBgp_mu_1, 5)
  expect_equal(dim(sdata$Xgp_mu_1), c(10, 5))
  expect_equal(dim(sdata$slambda_mu_1), c(5, 1))

  sdata <- SW(standata(y ~ gp(x1, by = z, k = 5, c = 5/4, scale = FALSE), dat))
  expect_equal(sdata$Igp_mu_1_2, as.array(4))
  expect_equal(sdata$Cgp_mu_1_2, as.array(1))
  expect_equal(sdata$Igp_mu_1_4, as.array(6:10))
})
...
test_that("standata includes data for CAR models", {
  ...
  expect_equal(sdata$Nloc_mu, 10)
  expect_equal(unname(sdata$Nneigh_mu), rep(1, 10))
  expect_equal(unname(sdata$edges1_mu), as.array(10:6))
  expect_equal(unname(sdata$edges2_mu), as.array(1:5))
  ...
  sdata <- standata(y ~ x + car(W, gr = group), dat, data2 = dat2,
                    prior = prior(horseshoe(), class = sdcar, dpar = mu))
  expect_equal(sdata$Nloc_mu, 2)
  expect_equal(sdata$edges1_mu, as.array(2))
  expect_equal(sdata$edges2_mu, as.array(1))
  expect_equal(sdata$Kscales_mu, 1)

  sdata <- standata(y ~ x + car(W, group, type = "bym2"),
                    data = dat, data2 = dat2)
  expect_equal(length(sdata$car_scale_mu), 1L)
...
test_that("standata includes data of special priors", {
  ...
  expect_equal(sdata$hs_df_mu, 7)
  expect_equal(sdata$hs_df_global_mu, 3)
  expect_equal(sdata$hs_df_slab_mu, 6)
  expect_equal(sdata$hs_scale_global_mu, 2)
  expect_equal(sdata$hs_scale_slab_mu, 3)

  hs <- horseshoe(par_ratio = 0.1)
  sdata <- standata(y ~ x1*x2, data = dat, prior = set_prior(hs, dpar = 'mu'))
  expect_equal(sdata$hs_scale_global_mu, 0.1 / sqrt(nrow(dat)))

  # R2D2 prior
  sdata <- standata(y ~ x1*x2, data = dat,
                    prior = prior(R2D2(0.5, 10), dpar = mu))
  expect_equal(sdata$R2D2_mean_R2_mu, 0.5)
  expect_equal(sdata$R2D2_prec_R2_mu, 10)
  expect_equal(sdata$R2D2_cons_D2_mu, as.array(rep(0.5, 3)))
  ...
  bform <- bf(y ~ x1*mo(x3) + (1|g) + gp(x3) + s(x2) +
                arma(p = 2, q = 2, gr = g))
  bprior <- prior(R2D2(cons_D2 = 11:1, main = TRUE), class = b, dpar = mu) +
  ...
  expect_equal(sdata$Kscales_mu, 11)
  expect_equal(sdata$R2D2_cons_D2_mu, as.array(11:1))
})
...
test_that("standata handles cox models correctly", {
  ...
  bform <- bf(y ~ x)
  bprior <- prior(dirichlet(3), sbhaz, dpar = mu)
