context("Tests for stancode with keep_mu = TRUE")
...
op <- options(brms.parse_stancode = not_cran, brms.backend = "rstan",
              brms.keep_mu = TRUE)
on.exit(op)
...
test_that("specified priors appear in the Stan code", {
  ...
  prior <- c(prior(std_normal(), coef = x1, dpar = mu),
             prior(normal(0,2), coef = x2, dpar = mu),
             prior(normal(0,5), Intercept, lb = 0, dpar = mu),
             prior(cauchy(0,1), sd, group = g, lb = "", ub = 5, dpar = mu),
             prior(cauchy(0,2), sd, group = g, coef = x1, dpar = mu),
             prior(gamma(1, 1), sd, group = h, ub = 10, dpar = mu))
  ...
  expect_match2(scode, "lprior += std_normal_lpdf(b_mu[1])")
  expect_match2(scode, "lprior += normal_lpdf(b_mu[2] | 0, 2)")
  expect_match2(scode, "lprior += normal_lpdf(Intercept_mu | 0, 5)")
  ...
  expect_match2(scode, "prior_b_mu__1 = normal_rng(0,1);")
  ...
  prior <- c(prior(lkj(0.5), class = cor, group = g),
             ...
             prior(cauchy(0, 5), class = sd, dpar = mu))
  scode <- stancode(y ~ x1 + cs(x2) + (0 + x1 + x2 | g),
                    data = dat, family = acat(),
                    prior = prior, sample_prior = TRUE)
  expect_match2(scode, "lprior += normal_lpdf(b_mu | 0, 1)")
  expect_match2(scode, "lprior += normal_lpdf(Intercept_mu | 0, 5)")
  expect_match2(scode, "lprior += cauchy_lpdf(sd_1 | 0, 5)")
  expect_match2(scode, "lprior += lkj_corr_cholesky_lpdf(L_1 | 0.5)")
  expect_match2(scode, "lprior += normal_lpdf(to_vector(bcs_mu) | 0, 1)")
  expect_match2(scode, "prior_bcs_mu = normal_rng(0,1)")
  ...
  prior <- c(prior(uniform(-1, 1), ar, dpar = mu),
             prior(normal(0, 0.5), ma, dpar = mu),
             prior(normal(0, 5), dpar = mu))
  scode <- stancode(y ~ mo(g) + arma(cov = TRUE), dat,
                    prior = prior, sample_prior = TRUE)
  expect_match2(scode, "vector<lower=-1,upper=1>[Kar_mu] ar_mu;")
  expect_match2(scode, "vector<lower=-1,upper=1>[Kma_mu] ma_mu;")
  expect_match2(scode, "lprior += uniform_lpdf(ar_mu | -1, 1)")
  expect_match2(scode, "lprior += normal_lpdf(ma_mu | 0, 0.5)")
  expect_match2(scode,
                "- 1 * log_diff_exp(normal_lcdf(1 | 0, 0.5), normal_lcdf(-1 | 0, 0.5))"
  )
  expect_match2(scode, "lprior += normal_lpdf(bsp_mu | 0, 5)")
  expect_match2(scode, "lprior += dirichlet_lpdf(simo_mu_1 | con_simo_mu_1)")
  expect_match2(scode, "prior_simo_mu_1 = dirichlet_rng(con_simo_mu_1)")
  expect_match2(scode, "prior_ar_mu = uniform_rng(-1,1)")
  expect_match2(scode, "while (prior_ar_mu < -1 || prior_ar_mu > 1)")

  # test for problem described in #213
  prior <- c(prior(normal(0, 1), coef = x1, dpar = mu),
             prior(normal(0, 2), coef = x1, dpar = sigma))
  scode <- stancode(bf(y ~ x1, sigma ~ x1), dat, prior = prior)
  expect_match2(scode, "lprior += normal_lpdf(b_mu[1] | 0, 1);")
  expect_match2(scode, "lprior += normal_lpdf(b_sigma[1] | 0, 2);")

  prior <- c(set_prior("target += normal_lpdf(b_mu[1] | 0, 1)", check = FALSE),
             set_prior("", class = "sigma"))
  scode <- stancode(y ~ x1, dat, prior = prior, sample_prior = TRUE)
  expect_match2(scode, "target += normal_lpdf(b_mu[1] | 0, 1)")
  expect_true(!grepl("sigma \\|", scode))

  # commented out until fixes implemented in 'check_prior_content'
  prior <- prior(gamma(0, 1), coef = x1, dpar = mu)
  expect_warning(stancode(y ~ x1, dat, prior = prior),
                 "no natural lower bound")
  prior <- prior(uniform(0,5), class = sd, dpar = mu)
...
test_that("special shrinkage priors appear in the Stan code", {
  ...
  scode <- stancode(y ~ x1*x2, data = dat,
                    prior = set_prior(hs, dpar = 'mu'),
                    sample_prior = TRUE)
  expect_match2(scode, "vector<lower=0>[Kscales_mu] hs_local_mu;")
  expect_match2(scode, "real<lower=0> hs_global_mu;")
  expect_match2(scode,
                "target += student_t_lpdf(hs_local_mu | hs_df_mu, 0, 1)"
  )
  expect_match2(scode,
                "lprior += student_t_lpdf(hs_global_mu | hs_df_global_mu, 0, hs_scale_global_mu * sigma)"
  )
  expect_match2(scode,
                "lprior += inv_gamma_lpdf(hs_slab_mu | 0.5 * hs_df_slab_mu, 0.5 * hs_df_slab_mu)"
  )
  expect_match2(scode,
                "scales_mu = scales_horseshoe(hs_local_mu, hs_global_mu, hs_scale_slab_mu^2 * hs_slab_mu);"
  )

  scode <- stancode(y ~ x1*x2, data = dat, poisson(),
                    prior = prior(horseshoe(scale_global = 3), dpar = mu))
  expect_match2(scode,
                "scales_mu = scales_horseshoe(hs_local_mu, hs_global_mu, hs_scale_slab_mu^2 * hs_slab_mu);"
  )

  scode <- stancode(x1 ~ mo(y), dat, prior = prior(horseshoe(), dpar= mu))
  expect_match2(scode, "target += std_normal_lpdf(zbsp_mu);")
  expect_match2(scode,
                "target += student_t_lpdf(hs_local_mu | hs_df_mu, 0, 1)"
  )
  expect_match2(scode,
  ...
  # R2D2 prior
  scode <- stancode(y ~ x1*x2, data = dat,
                    prior = prior(R2D2(0.5, 10), dpar = 'mu'),
                    sample_prior = TRUE)
  expect_match2(scode, "scales_mu = scales_R2D2(R2D2_phi_mu, R2D2_tau2_mu);")
  expect_match2(scode, "target += dirichlet_lpdf(R2D2_phi_mu | R2D2_cons_D2_mu);")
  expect_match2(scode, "lprior += beta_lpdf(R2D2_R2_mu | R2D2_mean_R2_mu * R2D2_prec_R2_mu, (1 - R2D2_mean_R2_mu) * R2D2_prec_R2_mu);")
  expect_match2(scode, "R2D2_tau2_mu = sigma^2 * R2D2_R2_mu / (1 - R2D2_R2_mu);")
  ...
  bform <- bf(y ~ x1*mo(x3) + (1|g) + (1|x1) + gp(x3) + s(x2) +
                arma(p = 2, q = 2, gr = g))
  bprior <- prior(R2D2(main = TRUE), class = b, dpar = mu) +
    prior(R2D2(), class = sd, dpar = mu) +
    prior(R2D2(), class = sds, dpar = mu) +
    prior(R2D2(), class = sdgp, dpar = mu) +
    prior(R2D2(), class = ar, dpar = mu) +
    prior(R2D2(), class = ma, dpar = mu)
  scode <- stancode(bform, data = dat, prior = bprior)
  expect_match2(scode, "sdb_mu = scales_mu[(1):(Kc_mu)];")
  expect_match2(scode, "sdbsp_mu = scales_mu[(1+Kc_mu):(Kc_mu+Ksp_mu)];")
  expect_match2(scode, "sdbs_mu = scales_mu[(1+Kc_mu+Ksp_mu):(Kc_mu+Ksp_mu+Ks_mu)];")
  expect_match2(scode, "sds_mu_1 = scales_mu[(1+Kc_mu+Ksp_mu+Ks_mu):(Kc_mu+Ksp_mu+Ks_mu+nb_mu_1)];")
  expect_match2(scode, "sdgp_mu_1 = scales_mu[(1+Kc_mu+Ksp_mu+Ks_mu+nb_mu_1):(Kc_mu+Ksp_mu+Ks_mu+nb_mu_1+Kgp_mu_1)];")
  expect_match2(scode, "sdar_mu = scales_mu[(1+Kc_mu+Ksp_mu+Ks_mu+nb_mu_1+Kgp_mu_1):(Kc_mu+Ksp_mu+Ks_mu+nb_mu_1+Kgp_mu_1+Kar_mu)];")
  expect_match2(scode, "sdma_mu = scales_mu[(1+Kc_mu+Ksp_mu+Ks_mu+nb_mu_1+Kgp_mu_1+Kar_mu):(Kc_mu+Ksp_mu+Ks_mu+nb_mu_1+Kgp_mu_1+Kar_mu+Kma_mu)];")
  expect_match2(scode, "sd_1 = scales_mu[(1+Kc_mu+Ksp_mu+Ks_mu+nb_mu_1+Kgp_mu_1+Kar_mu+Kma_mu):(Kc_mu+Ksp_mu+Ks_mu+nb_mu_1+Kgp_mu_1+Kar_mu+Kma_mu+M_1)];")
  expect_match2(scode, "sd_2 = scales_mu[(1+Kc_mu+Ksp_mu+Ks_mu+nb_mu_1+Kgp_mu_1+Kar_mu+Kma_mu+M_1):(Kc_mu+Ksp_mu+Ks_mu+nb_mu_1+Kgp_mu_1+Kar_mu+Kma_mu+M_1+M_2)];")
  expect_match2(scode, "bsp_mu = zbsp_mu .* sdbsp_mu;  // scale coefficients")
  expect_match2(scode, "ar_mu = zar_mu .* sdar_mu;  // scale coefficients")

  # check error messages
  expect_error(stancode(y ~ x1*x2, data = dat,
                        prior = prior(horseshoe(-1), dpar = mu)),
               "Degrees of freedom of the local priors")
  expect_error(stancode(y ~ x1*x2, data = dat,
                        prior = prior(horseshoe(1, -1), dpar = mu)),
               "Scale of the global prior")
  expect_error(stancode(y ~ cs(x1), dat, acat(), prior = prior(R2D2(), dpar = mu)),
               "Special priors are not yet allowed")
  bprior <- prior(horseshoe(), dpar = mu) + prior(normal(0, 1), coef = "y", dpar = mu)
  expect_error(stancode(x1 ~ y, dat, prior = bprior),
               "Defining separate priors for single coefficients")
  expect_error(stancode(x1 ~ y, dat, prior = prior(horseshoe(), lb = 0, dpar = mu)),
               "Setting boundaries on coefficients is not allowed")
  expect_error(
    stancode(y ~ x1*x2, data = dat, prior = prior(lasso(2, scale = 10), dpar = mu)),
...
test_that("priors can be fixed to constants", {
  ...
  prior <- prior(normal(0, 1), b, dpar = mu) +
    prior(constant(3), b, coef = x1, dpar = mu) +
    prior(constant(-1), b, coef = x2, dpar = mu) +
    prior(constant(10), Intercept, dpar = mu) +
    prior(normal(0, 5), sd, dpar = mu) +
    prior(constant(1), sd, group = g, coef = x2, dpar = mu) +
    prior(constant(2), sd, group = g, coef = x1, dpar = mu) +
    prior(constant(0.3), sigma)
  scode <- stancode(y ~ x1*x2 + (x1*x2 | g), dat, prior = prior)
  expect_match2(scode, "b_mu[1] = 3;")
  expect_match2(scode, "b_mu[2] = -1;")
  expect_match2(scode, "b_mu[3] = par_b_mu_3;")
  expect_match2(scode, "lprior += normal_lpdf(b_mu[3] | 0, 1);")
  expect_match2(scode, "Intercept_mu = 1")
  ...
  prior <- prior(constant(3), dpar = mu)
  scode <- stancode(y ~ x2 + x1 + cs(g), dat, family = sratio(),
                    prior = prior)
  expect_match2(scode, "b_mu = rep_vector(3, rows(b_mu));")
  expect_match2(scode, "bcs_mu = rep_matrix(3, rows(bcs_mu), cols(bcs_mu));")

  prior <- prior(normal(0, 3), dpar = mu) +
    prior(constant(3), coef = x1, dpar = mu) +
    prior(constant(-1), coef = g, dpar = mu)
  scode <- stancode(y ~ x1 + cs(x2) + cs(g), dat, family = sratio(),
                    prior = prior)
  expect_match2(scode, "b_mu[1] = 3;")
  expect_match2(scode, "bcs_mu[1] = par_bcs_mu_1;")
  expect_match2(scode, "lprior += normal_lpdf(bcs_mu[1] | 0, 3);")
  expect_match2(scode, "bcs_mu[2] = rep_row_vector(-1, cols(bcs_mu[2]));")

  prior <- prior(constant(3), class = "sd", group = "g", dpar = mu) +
  ...
  prior <- prior(constant(0.5), class = lscale, coef = gpx1h1, dpar = mu) +
    prior(normal(0, 10), class = lscale, coef = gpx1h2, dpar = mu)
  scode <- stancode(y ~ gp(x1, by = h), dat, prior = prior)
  expect_match2(scode, "lscale_mu_1[1][1] = 0.5;")
  expect_match2(scode, "lscale_mu_1[2][1] = par_lscale_mu_1_2_1;")
  expect_match2(scode, "lprior += normal_lpdf(lscale_mu_1[2][1] | 0, 10)")

  # test that improper base priors are correctly recognized (#919)
  prior <- prior(constant(-1), b, coef = x2, dpar = mu)
  scode <- stancode(y ~ x1*x2, dat, prior = prior)
  expect_match2(scode, "real par_b_mu_1;")
  expect_match2(scode, "b_mu[3] = par_b_mu_3;")

  # test error messages
  prior <- prior(normal(0, 1), Intercept, dpar = mu) +
    prior(constant(3), Intercept, coef = 2, dpar = mu)
  expect_error(
...
test_that("Stan GLM primitives are applied correctly", {
  ...
  expect_match2(scode, "normal_id_glm_lpdf(Y | Xc_mu, Intercept_mu, b_mu, sigma)")

  scode <- stancode(y ~ x, dat, family = bernoulli)
  expect_match2(scode, "bernoulli_logit_glm_lpmf(Y | Xc_mu, Intercept_mu, b_mu)")

  scode <- stancode(y ~ x, dat, family = poisson)
  expect_match2(scode, "poisson_log_glm_lpmf(Y | Xc_mu, Intercept_mu, b_mu)")

  scode <- stancode(y ~ x, dat, family = negbinomial)
  expect_match2(scode,
                "neg_binomial_2_log_glm_lpmf(Y | Xc_mu, Intercept_mu, b_mu, shape)"
  )

  scode <- stancode(y ~ x, dat, family = brmsfamily("negbinomial2"))
  expect_match2(scode,
                "neg_binomial_2_log_glm_lpmf(Y | Xc_mu, Intercept_mu, b_mu, inv(sigma))"
  )

  scode <- stancode(y ~ 0 + x, dat, family = gaussian)
  expect_match2(scode, "normal_id_glm_lpdf(Y | X_mu, 0, b_mu, sigma)")

  bform <- bf(y ~ x) + bf(x ~ 1, family = negbinomial()) + set_rescor(FALSE)
  scode <- stancode(bform, dat, family = gaussian)
  expect_match2(scode,
                "normal_id_glm_lpdf(Y_y | Xc_mu_y, Intercept_mu_y, b_mu_y, sigma_y)"
  )

  scode <- stancode(bf(y ~ x, decomp = "QR"), dat, family = gaussian)
  expect_match2(scode, "normal_id_glm_lpdf(Y | XQ_mu, Intercept_mu, bQ_mu, sigma);")
})
...
test_that("customized covariances appear in the Stan code", {
  ...
  expect_match2(scode, "r_1_mu_1 = (sd_1[1] * (Lcov_1 * z_1[1]))")
  ...
  scode <- stancode(rating ~ (1 + treat | gr(subject, cor = FALSE, cov = M)),
                    data = inhaler, data2 = dat2)
  expect_match2(scode, "r_1_mu_1 = (sd_1[1] * (Lcov_1 * z_1[1]));")
  expect_match2(scode, "r_1_mu_2 = (sd_1[2] * (Lcov_1 * z_1[2]));")
...
test_that("stancode correctly restricts FE parameters", {
  ...
  scode <- stancode(y ~ x, data, prior = set_prior("", lb = 2, dpar = 'mu'))
  expect_match2(scode, "vector<lower=2>[Kc_mu] b_mu")

  scode <- stancode(
    y ~ x, data, prior = set_prior("normal (0, 2)", ub = "4", dpar = 'mu')
  )
  expect_match2(scode, "vector<upper=4>[Kc_mu] b_mu")
  expect_match2(scode, "- 1 * normal_lcdf(4 | 0, 2)")

  prior <- set_prior("normal(0,5)", lb = "-3", ub = 5, dpar = 'mu')
  scode <- stancode(y ~ 0 + x, data, prior = prior)
  expect_match2(scode, "vector<lower=-3,upper=5>[K_mu] b_mu")
})
...
test_that("Stan code for multivariate models is correct", {
  ...
  prior <- prior(horseshoe(2), resp = "y1", dpar = mu) +
    prior(horseshoe(2), resp = "y2", dpar = mu)
  scode <- stancode(form, dat, prior = prior)
  ...
  expect_match2(scode, "target += student_t_lpdf(hs_local_mu_y1 | hs_df_mu_y1, 0, 1)")
  expect_match2(scode, "target += student_t_lpdf(hs_local_mu_y2 | hs_df_mu_y2, 0, 1)")
  expect_match2(scode, "rescor[choose(k - 1, 2) + j] = Rescor[j, k];")

  form <- bf(mvbind(y1, y2) ~ x) + set_rescor(TRUE)
  prior <- prior(R2D2(0.2, 10), resp = "y1", dpar = mu) +
    prior(R2D2(0.5, 10), resp = "y2", dpar = mu)
  ...
  expect_match2(scode, "target += dirichlet_lpdf(R2D2_phi_mu_y2 | R2D2_cons_D2_mu_y2);")
  ...
  bprior <- prior(normal(0, 5), resp = y1, dpar = mu) +
    prior(normal(0, 10), resp = y2, dpar = mu)
  scode <- stancode(bform, dat, prior = bprior)
  expect_match2(scode, "r_1_mu_y2_3 = r_1[, 3]")
  expect_match2(scode, "err_mu_y1[n] = Y_y1[n] - mu_y1[n]")
  ...
  expect_match2(scode, "lprior += normal_lpdf(b_mu_y1 | 0, 5)")
  expect_match2(scode, "lprior += normal_lpdf(bs_mu_y2 | 0, 10)")
...
test_that("Stan code for ARMA models is correct", {
  ...
  expect_match2(scode, "vector[Kar_mu] ar_mu")
  expect_match2(scode, "err_mu[n] = Y[n] - mu[n];")
  expect_match2(scode, "mu[n] += Err_mu[n, 1:Kar_mu] * ar_mu;")

  scode <- stancode(y ~ x + ma(time, q = 2), dat, student())
  expect_match2(scode, "mu[n] += Err_mu[n, 1:Kma_mu] * ma_mu;")
  ...
  expect_match2(scode, "err_mu_y[n] = Y_y[n] - mu_y[n];")
  ...
  scode <- stancode(
    ...
    prior = prior(cauchy(0, 10), class = sderr, dpar = mu)
  )
  expect_match2(scode, "Lcortime_mu = cholesky_cor_ar1(ar_mu[1], max_nobs_tg_mu);")
  expect_match2(scode,
                "err_mu = scale_time_err(zerr_mu, sderr_mu, Lcortime_mu, nobs_tg_mu, begin_tg_mu, end_tg_mu);"
  )
  expect_match2(scode, "mu += Intercept_mu + Xc_mu * b_mu + err_mu;")
  expect_match2(scode, "lprior += cauchy_lpdf(sderr_mu | 0, 10)")
  ...
  expect_match2(scode, "vector<lower=-1,upper=1>[Kar_mu] ar_mu")
  expect_match2(scode, "mu[n] += Err_mu[n, 1:Kar_mu] * ar_mu;")
  expect_match2(scode, "err_mu = sderr_mu * zerr_mu;")
  expect_match2(scode, "mu += Intercept_mu + Xc_mu * b_mu + err_mu;")
  expect_match2(scode, "lprior += cauchy_lpdf(sderr_mu | 0, 10)")
  ...
  scode <- stancode(
    y ~ x + ar(time), dat, family = poisson,
    prior = prior(horseshoe(main = TRUE), class = b, dpar = mu) +
      prior(horseshoe(), class = sderr, dpar = mu)
  )
  expect_match2(scode, "sderr_mu = scales_mu[(1+Kc_mu):(Kc_mu+1)][1];")
})
...
test_that("Stan code for compound symmetry models is correct", {
  ...
  scode <- stancode(
    y ~ x + cosy(time), dat,
    prior = prior(normal(0, 2), cosy, dpar = mu)
  )
  expect_match2(scode, "real<lower=0,upper=1> cosy_mu;")
  expect_match2(scode, "Lcortime_mu = cholesky_cor_cosy(cosy_mu, max_nobs_tg_mu);")
  expect_match2(scode, "lprior += normal_lpdf(cosy_mu | 0, 2)")

  scode <- stancode(bf(y ~ x + cosy(time), sigma ~ x), dat)
  expect_match2(scode, "normal_time_het_lpdf(Y | mu, sigma, Lcortime_mu")
...
test_that("Stan code for UNSTR covariance terms is correct", {
  ...
  expect_match2(scode, "normal_time_hom_flex_lpdf(Y | mu, sigma, Lcortime_mu, nobs_tg_mu, begin_tg_mu, end_tg_mu, Jtime_tg_mu);")
  expect_match2(scode, "cortime_mu[choose(k - 1, 2) + j] = Cortime_mu[j, k];")
  expect_match2(scode, "lprior += lkj_corr_cholesky_lpdf(Lcortime_mu | 1);")

  scode <- stancode(
    y ~ x + unstr(tim, g), data = dat,
    family = student(), prior = prior(lkj(4), cortime, dpar = mu)
  )
  expect_match2(scode, "student_t_time_hom_flex_lpdf(Y | nu, mu, sigma, Lcortime_mu, nobs_tg_mu, begin_tg_mu, end_tg_mu, Jtime_tg_mu);")
  expect_match2(scode, "lprior += lkj_corr_cholesky_lpdf(Lcortime_mu | 4);")
  ...
  expect_match2(scode, "normal_time_hom_se_flex_lpdf(Y | mu, sigma, se2, Lcortime_mu, nobs_tg_mu, begin_tg_mu, end_tg_mu, Jtime_tg_mu);")
  ...
  expect_match2(scode, "err_mu = scale_time_err_flex(zerr_mu, sderr_mu, Lcortime_mu, nobs_tg_mu, begin_tg_mu, end_tg_mu,")
  expect_match2(scode, "mu += Intercept_mu + Xc_mu * b_mu + err_mu;")
...
test_that("Stan code of ordinal models is correct", {
  ...
  scode <- stancode(
    ...
    prior = prior(normal(0, 2), Intercept, coef = 2, dpar = mu)
  )
  expect_match2(scode,
                "target += ordered_logistic_lpmf(Y[n] | mu[n], Intercept_mu);"
  )
  expect_match2(scode, "lprior += student_t_lpdf(Intercept_mu[1] | 3, 0, 2.5);")
  expect_match2(scode, "lprior += normal_lpdf(Intercept_mu[2] | 0, 2);")

  scode <- stancode(
    y ~ x1, dat, cumulative("probit", threshold = "equidistant"),
    prior = prior(normal(0, 2), Intercept, dpar = mu)
  )
  expect_match2(scode, "real cumulative_probit_lpmf(int y")
  expect_match2(scode, "p = Phi(disc * (thres[1] - mu));")
  expect_match2(scode, "real<lower=0> delta_mu;")
  expect_match2(scode, "Intercept_mu[k] = first_Intercept_mu + (k - 1.0) * delta_mu;")
  expect_match2(scode, "b_mu_Intercept = Intercept_mu + dot_product(means_X_mu, b_mu);")
  expect_match2(scode, "lprior += normal_lpdf(first_Intercept_mu | 0, 2);")
  ...
  expect_match2(scode, "matrix[N, Kcs_mu] Xcs_mu;")
  expect_match2(scode, "matrix[Kcs_mu, nthres] bcs_mu;")
  expect_match2(scode, "mucs_mu = Xcs_mu * bcs_mu;")
  expect_match2(scode,
                "target += sratio_logit_lpmf(Y[n] | mu[n], disc, Intercept_mu - transpose(mucs_mu[n]));"
  )

  scode <- stancode(y ~ x1 + cse(x2) + (cse(1)|g), dat, family = acat())
  expect_match2(scode, "real acat_logit_lpmf(int y")
  expect_match2(scode, "mucs_mu[n, 1] = mucs_mu[n, 1] + r_1_mu_1[J_1[n]] * Z_1_mu_1[n];")
  expect_match2(scode, "b_mu_Intercept = Intercept_mu + dot_product(means_X_mu, b_mu);")

  scode <- stancode(y ~ x1 + (cse(x2)||g), dat, family = acat("probit_approx"))
  expect_match2(scode,
                paste("mucs_mu[n, 3] = mucs_mu[n, 3] + r_1_mu_3[J_1[n]] * Z_1_mu_3[n]",
                      "+ r_1_mu_6[J_1[n]] * Z_1_mu_6[n];"))
  expect_match2(scode,
                "target += acat_probit_approx_lpmf(Y[n] | mu[n], disc, Intercept_mu - transpose(mucs_mu[n]));"
  )
  ...
  expect_match2(scode, "Intercept_mu_stz = Intercept_mu - mean(Intercept_mu);")
  expect_match2(scode, "cumulative_probit_lpmf(Y[n] | mu[n], disc, Intercept_mu_stz);")
  expect_match2(scode, "vector[nthres] b_mu_Intercept = Intercept_mu_stz;")
...
test_that("ordinal disc parameters appear in the Stan code", {
  ...
  expect_match2(scode,
                "target += cumulative_logit_lpmf(Y[n] | mu[n], disc[n], Intercept_mu)"
  )
...
test_that("grouped ordinal thresholds appear in the Stan code", {
  ...
  prior <- prior(normal(0,1), class = "Intercept", group = "b", dpar = mu)
  scode <- stancode(
    y | thres(th, gr) ~ x, data = dat,
    family = sratio(), prior = prior
  )
  expect_match2(scode, "array[ngrthres] int<lower=1> nthres;")
  expect_match2(scode, "merged_Intercept_mu[Kthres_start[1]:Kthres_end[1]] = Intercept_mu_1;")
  expect_match2(scode, "target += sratio_logit_merged_lpmf(Y[n]")
  expect_match2(scode, "lprior += normal_lpdf(Intercept_mu_2 | 0, 1);")
  # centering needs to be deactivated automatically
  expect_match2(scode, "vector[nthres[1]] b_mu_Intercept_1 = Intercept_mu_1;")
  ...
  expect_match2(scode, "real first_Intercept_mu_1;")
  expect_match2(scode, "lprior += normal_lpdf(first_Intercept_mu_2 | 0, 1);")
  expect_match2(scode, "Intercept_mu_2[k] = first_Intercept_mu_2 + (k - 1.0) * delta_mu_2;")
  ...
  expect_match2(scode, "merged_Intercept_mu_stz[Kthres_start[2]:Kthres_end[2]] = Intercept_mu_stz_2;")
  expect_match2(scode, "ordered_logistic_merged_lpmf(Y[n] | mu[n], merged_Intercept_mu_stz, Jthres[n]);")
  ...
  expect_match2(scode, "lprior += student_t_lpdf(Intercept_mu_y2_1 | 3, 0, 2.5);")
  expect_match2(scode, "merged_Intercept_mu_y[Kthres_start_y[2]:Kthres_end_y[2]] = Intercept_mu_y_2;")
})
...
test_that("Stan code of hurdle cumulative model is correct", {
  ...
  expect_match2(scode,
                "target += hurdle_cumulative_ordered_logistic_lpmf(Y[n] | mu[n], hu, disc, Intercept_mu);"
  )
  ...
  expect_match2(scode, "Intercept_mu_stz = Intercept_mu - mean(Intercept_mu);")
  expect_match2(scode, "hurdle_cumulative_cloglog_lpmf(Y[n] | mu[n], hu[n], disc[n], Intercept_mu_stz);")
  expect_match2(scode, "vector[nthres] b_mu_Intercept = Intercept_mu_stz;")
...
test_that("monotonic effects appear in the Stan code", {
  ...
  prior <- c(prior(normal(0,1), class = b, coef = mox1, dpar = mu),
             prior(dirichlet(c(1,0.5,2)), simo, coef = mox11, dpar = mu),
             prior(dirichlet(c(1,0.5,2)), simo, coef = mox21, dpar = mu))
  scode <- stancode(y ~ y*mo(x1)*mo(x2), dat, prior = prior)
  expect_match2(scode, "array[N] int Xmo_mu_3;")
  expect_match2(scode, "simplex[Jmo_mu[1]] simo_mu_1;")
  expect_match2(scode, "(bsp_mu[2]) * mo(simo_mu_2, Xmo_mu_2[n])")
  expect_match2(scode,
                "(bsp_mu[6]) * mo(simo_mu_7, Xmo_mu_7[n]) * mo(simo_mu_8, Xmo_mu_8[n]) * Csp_mu_3[n]"
  )
  expect_match2(scode, "lprior += normal_lpdf(bsp_mu[1] | 0, 1)")
  expect_match2(scode, "lprior += dirichlet_lpdf(simo_mu_1 | con_simo_mu_1);")
  expect_match2(scode, "lprior += dirichlet_lpdf(simo_mu_8 | con_simo_mu_8);")

  scode <- stancode(y ~ mo(x1) + (mo(x1) | x2), dat)
  expect_match2(scode, "(bsp_mu[1] + r_1_mu_2[J_1[n]]) * mo(simo_mu_1, Xmo_mu_1[n])")
  expect_true(!grepl("Z_1_w", scode))

  # test issue reported in discourse post #12978
  scode <- stancode(y ~ mo(x1) + (mo(x1) | x2) + (mo(x1) | g), dat)
  expect_match2(scode, "(bsp_mu[1] + r_1_mu_2[J_1[n]] + r_2_mu_2[J_2[n]]) * mo(simo_mu_1, Xmo_mu_1[n])")

  # test issue #813
  scode <- stancode(y ~ mo(x1):y, dat)
  expect_match2(scode, "mu[n] += (bsp_mu[1]) * mo(simo_mu_1, Xmo_mu_1[n]) * Csp_mu_1[n];")

  # test issue #924 (conditional monotonicity)
  prior <- c(prior(dirichlet(c(1,0.5,2)), simo, coef = "v", dpar = mu),
             prior(dirichlet(c(1,0.5,2)), simo, coef = "w", dpar = mu))
  scode <- stancode(y ~ y*mo(x1, id = "v")*mo(x2, id = "w"),
                    dat, prior = prior)
  expect_match2(scode, "lprior += dirichlet_lpdf(simo_mu_1 | con_simo_mu_1);")
  expect_match2(scode, "lprior += dirichlet_lpdf(simo_mu_2 | con_simo_mu_2);")
  expect_match2(scode, "simplex[Jmo_mu[6]] simo_mu_6 = simo_mu_2;")
  expect_match2(scode, "simplex[Jmo_mu[7]] simo_mu_7 = simo_mu_1;")
  ...
  prior <- prior(beta(1, 1), simo, coef = mox11, dpar = mu)
  expect_error(
...
test_that("FCOR matrices appear in the Stan code", {
  ...
  expect_match2(stancode(y ~ fcor(V), data = data, family = gaussian(),
                         data2 = list(V = V)),
                "target += normal_fcor_hom_lpdf(Y | mu, sigma, Lfcor_mu);")
  expect_match2(stancode(y ~ fcor(V), data = data, family = student(),
                         data2 = list(V = V)),
                "target += student_t_fcor_hom_lpdf(Y | nu, mu, sigma, Lfcor_mu);")
})
...
test_that("Stan code for GAMMs is correct", {
  ...
  scode <- stancode(y ~ s(x) + (1|g), data = dat,
                    prior = set_prior("normal(0,2)", "sds", dpar = 'mu'))
  expect_match2(scode, "Zs_mu_1_1 * s_mu_1_1")
  expect_match2(scode, "matrix[N, knots_mu_1[1]] Zs_mu_1_1")
  expect_match2(scode, "target += std_normal_lpdf(zs_mu_1_1)")
  expect_match2(scode, "lprior += normal_lpdf(sds_mu_1 | 0,2)")
  ...
  scode <- stancode(
    ...
    prior = set_prior("normal(0,1)", "sds", dpar = "mu") +
      set_prior("normal(0,2)", "sds", coef = "t2(x, y)", dpar = "mu")
  )
  expect_match2(scode, "Zs_mu_2_2 * s_mu_2_2")
  expect_match2(scode, "matrix[N, knots_mu_2[2]] Zs_mu_2_2")
  expect_match2(scode, "target += std_normal_lpdf(zs_mu_2_2)")
  expect_match2(scode, "lprior += normal_lpdf(sds_mu_1 | 0,1)")
  expect_match2(scode, "lprior += normal_lpdf(sds_mu_2 | 0,2)")

  scode <- stancode(y ~ g + s(x, by = g), data = dat)
  expect_match2(scode, "vector[knots_mu_2[1]] zs_mu_2_1")
  expect_match2(scode, "s_mu_2_1 = sds_mu_2[1] * zs_mu_2_1")
})
...
test_that("Stan code of response times models is correct", {
  ...
  expect_match2(scode, "mu[n] += r_1_mu_1[J_1[n]] * Z_1_mu_1[n];")
...
test_that("Group IDs appear in the Stan code", {
  ...
  expect_match2(scode, "r_2_mu_1 = r_2[, 1]")
...
test_that("noise-free terms appear in the Stan code", {
  ...
  me_prior <- prior(normal(0,5), dpar = mu) +
  ...
  expect_match2(scode,
                "(bsp_mu[1]) * Xme_1[n] + (bsp_mu[2]) * Xme_2[n] + (bsp_mu[3]) * Xme_1[n] * Xme_2[n]"
  )
  expect_match2(scode, "(bsp_mu[6]) * Xme_1[n] * Xme_2[n] * Csp_mu_3[n]")
  ...
  expect_match2(scode, "(bsp_mu[1] + r_1_mu_3[J_1[n]]) * Xme_1[n]")
  expect_match2(scode, "(bsp_mu[2] + r_1_mu_4[J_1[n]]) * Xme_1[n] * Csp_mu_1[n]")
  ...
  scode <- stancode(
    ...
    data = dat, prior = prior(normal(0,5), dpar = mu)
  )
  expect_match2(scode, "mu[n] += (bsp_mu[1]) * Xme_1[n]")
  ...
  scode <- stancode(bform, dat)
  expect_match2(scode, "mu_y[n] += (bsp_mu_y[1]) * Xme_1[n]")
  expect_match2(scode, "mu_z[n] += (bsp_mu_z[1]) * Xme_1[n]")
  ...
  expect_match2(scode, "(bsp_mu[1] + r_1_mu_2[J_1[n]]) * Xme_1[Jme_1[n]]")
...
test_that("Stan code of multi-membership models is correct", {
  ...
  expect_match2(stancode(y ~ (1|mm(g1, g2)), data = dat),
                paste0(" W_1_1[n] * r_1_mu_1[J_1_1[n]] * Z_1_mu_1_1[n]",
                       " + W_1_2[n] * r_1_mu_1[J_1_2[n]] * Z_1_mu_1_2[n]")
  )
  expect_match2(stancode(y ~ (1+w1|mm(g1,g2)), data = dat),
                paste0(" W_1_1[n] * r_1_mu_2[J_1_1[n]] * Z_1_mu_2_1[n]",
                       " + W_1_2[n] * r_1_mu_2[J_1_2[n]] * Z_1_mu_2_2[n]")
  )
...
test_that("by variables in grouping terms are handled correctly", {
  ...
  expect_match2(scode, "r_1_mu_1 = (transpose(sd_1[1, Jby_1]) .* (z_1[1]));")
...
test_that("Group syntax | and || is handled correctly,", {
  ...
  expect_match2(scode, "r_1_mu_2 = (sd_1[2] * (z_1[2]));")
  expect_match2(scode, "r_2_mu_1 = r_2[, 1];")
...
test_that("offsets appear in the Stan code", {
  ...
  expect_match2(scode, "+ offsets_mu;")
...
test_that("prior only models are correctly checked", {
  data <- data.frame(y = rnorm(10), x = rnorm(10), c = 1)
  prior <- prior(normal(0, 5), b, dpar = mu) + prior("", Intercept, dpar = mu)
  expect_error(stancode(y ~ x, data, prior = prior,
                        sample_prior = "only"),
               "Sampling from priors is not possible")
  prior <- prior(normal(0, 5), b, dpar = mu) + prior(normal(0, 10), Intercept, dpar = mu)
  scode <- stancode(y ~ x, data, prior = prior,
                    sample_prior = "only")
  expect_match2(scode, "lprior += normal_lpdf(Intercept_mu | 0, 10)")
})
...
test_that("sparse matrix multiplication is applied correctly", {
  ...
  scode <- stancode(
    ...
    data, prior = prior(normal(0, 5), coef = "Intercept", dpar = mu)
  )
  expect_match2(scode, "wX_mu = csr_extract_w(X_mu);")
  expect_match2(scode,
                "mu += csr_matrix_times_vector(rows(X_mu), cols(X_mu), wX_mu, vX_mu, uX_mu, b_mu);"
  )
  expect_match2(scode,
  ...
  expect_match2(scode, "lprior += normal_lpdf(b_mu[1] | 0, 5);")
...
test_that("QR decomposition is included in the Stan code", {
  ...
  # simple priors
  scode <- stancode(bform, data, prior = prior(normal(0, 2), dpar = mu))
  expect_match2(scode, "XQ_mu = qr_thin_Q(Xc_mu) * sqrt(N - 1);")
  expect_match2(scode, "b_mu = XR_mu_inv * bQ_mu;")
  expect_match2(scode, "lprior += normal_lpdf(bQ_mu | 0, 2);")
  expect_match2(scode, "XQ_mu * bQ_mu")
  expect_match2(scode, "XR_sigma = qr_thin_R(X_sigma) / sqrt(N - 1);")

  # horseshoe prior
  scode <- stancode(bform, data, prior = prior(horseshoe(1), dpar = mu))
  expect_match2(scode, "target += std_normal_lpdf(zb_mu);")
  expect_match2(scode, "scales_mu = scales_horseshoe(")
  expect_match2(scode, "sdb_mu = scales_mu[(1):(Kc_mu)];")
  expect_match2(scode, "bQ_mu = zb_mu .* sdb_mu;")
})
...
test_that("Stan code for Gaussian processes is correct", {
  ...
  prior <- prior(gamma(0.1, 0.1), sdgp, dpar = mu) +
    prior(gamma(4, 2), sdgp, coef = gpx2x1, dpar = mu)
  scode <- stancode(y ~ gp(x1) + gp(x2, by = x1, gr = FALSE),
                    dat, prior = prior)
  expect_match2(scode, "lprior += inv_gamma_lpdf(lscale_mu_1[1]")
  expect_match2(scode, "lprior += gamma_lpdf(sdgp_mu_1 | 0.1, 0.1)")
  expect_match2(scode, "lprior += gamma_lpdf(sdgp_mu_2 | 4, 2)")
  expect_match2(scode, "gp_pred_mu_2 = gp(Xgp_mu_2, sdgp_mu_2[1], lscale_mu_2[1], zgp_mu_2);")
  expect_match2(scode, "Cgp_mu_2 .* gp_pred_mu_2;")

  prior <- prior + prior(normal(0, 1), lscale, coef = gpx1, dpar = mu)
  scode <- stancode(y ~ gp(x1) + gp(x2, by = x1, gr = TRUE),
                    data = dat, prior = prior)
  expect_match2(scode, "lprior += normal_lpdf(lscale_mu_1[1][1] | 0, 1)")
  expect_match2(scode, "gp_pred_mu_2 = gp(Xgp_mu_2, sdgp_mu_2[1], lscale_mu_2[1], zgp_mu_2);")
  expect_match2(scode, "+ Cgp_mu_2 .* gp_pred_mu_2[Jgp_mu_2]")

  # non-isotropic GP
  scode <- stancode(y ~ gp(x1, x2, by = z, iso = FALSE), data = dat)
  expect_match2(scode, "lprior += inv_gamma_lpdf(lscale_mu_1[1][2]")
  expect_match2(scode, "lprior += inv_gamma_lpdf(lscale_mu_1[4][2]")

  # Suppress Stan parser warnings that can currently not be avoided
  scode <- stancode(y ~ gp(x1, x2) + gp(x1, by = z, gr = FALSE),
                    dat, silent = TRUE)
  expect_match2(scode, "gp(Xgp_mu_1, sdgp_mu_1[1], lscale_mu_1[1], zgp_mu_1)")
  expect_match2(scode, "mu[Igp_mu_2_2] += Cgp_mu_2_2 .* gp_pred_mu_2_2;")
  ...
  expect_match2(scode, "lprior += inv_gamma_lpdf(lscale_mu_1")
  expect_match2(scode,
                "rgp_mu_1 = sqrt(spd_cov_exp_quad(slambda_mu_1, sdgp_mu_1[1], lscale_mu_1[1])) .* zgp_mu_1;"
  )
  expect_match2(scode, "Cgp_mu_2 .* gp_pred_mu_2[Jgp_mu_2]")
  ...
  # test warnings
  prior <- prior(normal(0, 1), lscale, dpar = mu)
  expect_warning(
...
test_that("Stan code for SAR models is correct", {
  ...
  scode <- stancode(
    y ~ x + sar(W), data = dat,
    prior = prior(normal(0.5, 1), lagsar, dpar = mu),
    data2 = dat2
  )
  expect_match2(scode,
                "target += normal_lagsar_lpdf(Y | mu, sigma, lagsar_mu, Msar_mu, eigenMsar_mu)"
  )
  expect_match2(scode, "lprior += normal_lpdf(lagsar_mu | 0.5, 1)")
  ...
  expect_match2(scode,
                "target += student_t_lagsar_lpdf(Y | nu, mu, sigma, lagsar_mu, Msar_mu, eigenMsar_mu)"
  )

  scode <- stancode(y ~ x + sar(W, type = "error"), data = dat,
                    data2 = dat2)
  expect_match2(scode,
                "target += normal_errorsar_lpdf(Y | mu, sigma, errorsar_mu, Msar_mu, eigenMsar_mu)"
  )

  scode <- stancode(
    y ~ x + sar(W, "error"), data = dat, family = student(),
    prior = prior(beta(2, 3), errorsar, dpar = mu),
    data2 = dat2
  )
  expect_match2(scode,
                "target += student_t_errorsar_lpdf(Y | nu, mu, sigma, errorsar_mu, Msar_mu, eigenMsar_mu)"
  )
  expect_match2(scode, "lprior += beta_lpdf(errorsar_mu | 2, 3)")
...
test_that("Stan code for CAR models is correct", {
  ...
  expect_match2(scode, "mu[n] += rcar_mu[Jloc_mu[n]]")
  expect_match2(scode, "rcar_mu[Nloc_mu] = - sum(zcar_mu)")

  scode <- stancode(y ~ x + car(W, type = "icar"), dat, data2 = dat2)
  expect_match2(scode, "target += -0.5 * dot_self(zcar_mu[edges1_mu] - zcar_mu[edges2_mu])")
  expect_match2(scode, "target += normal_lpdf(sum(zcar_mu) | 0, 0.001 * Nloc_mu)")
  expect_match2(scode, "mu[n] += rcar_mu[Jloc_mu[n]]")
  expect_match2(scode, "rcar_mu = zcar_mu * sdcar_mu")

  scode <- stancode(y ~ x + car(W, type = "bym2"), dat, data2 = dat2)
  expect_match2(scode, "target += -0.5 * dot_self(zcar_mu[edges1_mu] - zcar_mu[edges2_mu])")
  expect_match2(scode, "target += normal_lpdf(sum(zcar_mu) | 0, 0.001 * Nloc_mu)")
  expect_match2(scode, "mu[n] += rcar_mu[Jloc_mu[n]]")
  expect_match2(scode, "lprior += beta_lpdf(rhocar_mu | 1, 1)")
  expect_match2(scode, paste0(
    "rcar_mu = (sqrt(1 - rhocar_mu) * nszcar_mu + ",
    "sqrt(rhocar_mu * inv(car_scale_mu)) * zcar_mu) * sdcar_mu"
  ))
  ...
  scode <- stancode(bf(y ~ x + car(W)), dat, data2 = dat2,
                    prior = prior(horseshoe(main = TRUE), class = b, dpar = mu) +
                      prior(horseshoe(), class = sdcar, dpar = mu))
  expect_match2(scode, "sdcar_mu = scales_mu[(1+Kc_mu):(Kc_mu+1)][1];")
})
...
test_that("Stan code for missing value terms works correctly", {
  ...
  expect_match2(scode, "(bsp_mu_y[1]) * Yl_x[n] + (bsp_mu_y[2]) * Yl_x[n] * Csp_mu_y_1[n];")
  expect_match2(scode, "target += normal_id_glm_lpdf(Yl_x | Xc_mu_x, Intercept_mu_x, b_mu_x, sigma_x);")

  bform <- bf(y ~ mi(x) + (mi(x) | g)) + bf(x | mi() ~ 1) + set_rescor(FALSE)
  scode <- stancode(bform, dat)
  expect_match2(scode,
                "(bsp_mu_y[1] + r_1_mu_y_2[J_1_y[n]]) * Yl_x[n] + r_1_mu_y_1[J_1_y[n]] * Z_1_mu_y_1[n];"
  )
  ...
  scode <- stancode(bform, dat)
  expect_match2(scode, "(bsp_mu_y[3]) * Yl_x[n] * mo(simo_mu_y_2, Xmo_mu_y_2[n]);")
  ...
  expect_match2(scode,
                "(bsp_mu_y[1]) * (log(Yl_x[n])) + (bsp_mu_y[2]) * (log(Yl_x[n])) * Csp_mu_y_1[n]"
  )
...
test_that("Missing value terms can be combined with 'subset'", {
  ...
  bform <- bf(y ~ mi(x, idx = g1)*mi(z)) +
    ...
    set_rescor(FALSE)
  scode <- stancode(bform, dat)
  expect_match2(scode, "(bsp_mu_y[1]) * Yl_x[idxl_mu_y_x_1[n]]")
  expect_match2(scode, "(bsp_mu_y[2]) * Yl_z[n]")
  expect_match2(scode, "(bsp_mu_y[3]) * Yl_x[idxl_mu_y_x_1[n]] * Yl_z[n]")
  expect_match2(scode, "array[N_y] int idxl_mu_y_x_1;")
})
...
test_that("argument 'stanvars' is handled correctly", {
  bprior <- prior(normal(mean_intercept, 10), class = "Intercept", dpar = mu)
  mean_intercept <- 5
  ...
  bprior <- prior(multi_normal(M, V), class = "b", dpar = mu)
  ...
  bprior <- set_prior("normal(0, tau)", class = "b", dpar = 'mu') +
  ...
  expect_match2(scode, "lprior += normal_lpdf(b_mu | 0, tau);")
  ...
  scode <- stancode(count ~ 1, data = epilepsy, family = poisson(),
                    ...
                    parse = FALSE)
  expect_match2(scode,
                "partial_log_lik_lpmf(array[] int seq, int start, int end, data array[] int Y, real Intercept_mu, data real foo, real tau)"
  )
  expect_match2(scode,
                "reduce_sum(partial_log_lik_lpmf, seq, grainsize, Y, Intercept_mu, foo, tau)"
  )
  ...
  # stanvars <- stanvar(scode = "vector[Ksp] zbsp_mu;", block = "parameters") +
  #   stanvar(scode = "real<lower=0> tau;", block = "parameters") +
  #   stanvar(scode = "vector[Ksp] bsp_mu = zbsp_mu * tau;",
  #           block = "tparameters", name = "bsp_mu")
  # scode <- stancode(count ~ mo(Base), epilepsy, stanvars = stanvars)
  # expect_match2(scode, "vector[Ksp] bsp_mu = zbsp_mu * tau;")
})
...
test_that("centering design matrices can be changed correctly", {
  ...
  scode <- stancode(
    ...
    prior = prior(normal(0,1), coef = Intercept, dpar = mu)
  )
  expect_match2(scode, "mu += X_mu * b_mu;")
  expect_match2(scode, "lprior += normal_lpdf(b_mu[1] | 0, 1);")
...
test_that("to_vector() is correctly removed from prior of SD parameters", {
  ...
  bprior <- c(
    prior(normal(0, 0.1), class = sd, dpar = mu) ,
...
test_that("Dirichlet priors can be flexibly included", {
  ...
  bprior <- prior("dirichlet([1,2]')", class = "b", dpar = mu)
  scode <- stancode(y ~ x1 + x2, dat, prior = bprior)
  expect_match2(scode, "simplex[Kc_mu] b_mu;")
})
...
test_that("threaded Stan code is correct", {
  ...
  expect_match2(scode, "mu[n] += (bsp_mu[1]) * mo(simo_mu_1, Xmo_mu_1[nn])")
  ...
  expect_match2(scode, "matrix[N, nthres] mucs_mu = Xcs_mu[start:end] * bcs_mu;")
  expect_match2(scode,
                "ptarget += sratio_logit_lpmf(Y[nn] | mu[n], disc, Intercept_mu")
  expect_match2(scode, " - transpose(mucs_mu[n]));")
  ...
  expect_match2(scode,
                "ptarget += normal_id_glm_lpdf(Y_Exp[start:end] | Xc_mu_Exp[start:end], Intercept_mu_Exp, b_mu_Exp, sigma_Exp);"
  )
...
test_that("Un-normalized Stan code is correct", {
  ...
  scode <- stancode(
    ...
    data = epilepsy, family = poisson(),
    prior = prior(student_t(5,0,10), class = b, dpar = mu) +
      prior(cauchy(0,2), class = sd, dpar = mu),
    normalize = FALSE
  )
  expect_match2(scode, "target += poisson_log_glm_lupmf(Y | Xc_mu, mu, b_mu);")
  expect_match2(scode, "lprior += student_t_lupdf(b_mu | 5, 0, 10);")
  expect_match2(scode, "lprior += student_t_lupdf(Intercept_mu | 3, 1.4, 2.5);")
  ...
  expect_match2(scode, "target += reduce_sum(partial_log_lik_lpmf, seq, grainsize, Y, Xc_mu, b_mu,")
  expect_match2(scode, "Intercept_mu, J_1, Z_1_mu_1, r_1_mu_1, J_2, Z_2_mu_1, r_2_mu_1);")
  expect_match2(scode, "ptarget += poisson_log_glm_lupmf(Y[start:end] | Xc_mu[start:end], mu, b_mu);")
  expect_match2(scode, "lprior += student_t_lupdf(b_mu | 5, 0, 10);")
  expect_match2(scode, "lprior += student_t_lupdf(Intercept_mu | 3, 1.4, 2.5);")
  expect_match2(scode, "lprior += cauchy_lupdf(sd_1 | 0, 2);")
  expect_match2(scode, "target += std_normal_lupdf(z_1[1]);")
  ...
  expect_match2(scode, "target += sratio_cloglog_lpmf(Y[n] | mu[n], disc, Intercept_mu")
  expect_match2(scode, "- transpose(mucs_mu[n]));")
...
#     count ~ zAge + zBase * Trt + (1|patient) + (1|obs),
#     data = epilepsy, family = poisson(),
#     prior = prior(student_t(5,0,10), class = b_mu) +
