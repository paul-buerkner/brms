...
- Specify a prior `tag` for use in prior sensitivity analysis via
  `priorsense` thanks to Kallioinen.
  ([\#1585](https://github.com/paul-buerkner/brms/issues/1585))
- Specify group-level prior weights via argument `pw` in multilevel `gr`
  and `mm` terms thanks to Ben Schneider.
  ([\#1719](https://github.com/paul-buerkner/brms/issues/1719))
- Fit extended-support Beta models via family `xbeta` thanks to Ioannis
  Kosmidis.
  ([\#1698](https://github.com/paul-buerkner/brms/issues/1698))
- Add a `seed` argument to `loo_R2` thanks to Marco Colombo.
  ([\#1713](https://github.com/paul-buerkner/brms/issues/1713))
- Add family `dirichlet_multinomial` to fit overdispersed multinomial
  data thanks to Tom Peatman.
  ([\#1729](https://github.com/paul-buerkner/brms/issues/1729))
- Add the `int_step` R function to match the corresponding Stan function
  thanks to Daniel Sabanes Bove.
  ([\#1734](https://github.com/paul-buerkner/brms/issues/1734))
- Add a new global option `brms.cache_folder`, which allows users to
  define a default directory for saving and loading cached brmsfit
  objects. Thanks to Sermet Pekin.
  ([\#1790](https://github.com/paul-buerkner/brms/issues/1790))
- Extend `posterior_predict` beyond random draws via argument `output`,
  ...
  `lower.tail`, `log.p`, and `log` thanks to Aki Vehtari and Florence
  Bockting.
  ([\#1857](https://github.com/paul-buerkner/brms/issues/1857))
- Predict all mixing proportions of a `mixture` family without a
  reference category via `refcat = NA`, analogous to `refcat = NA` in
  categorical models. Thanks to Gidon Frischkorn.
  ([\#1450](https://github.com/paul-buerkner/brms/issues/1450))

#### Bug Fixes

- Return `-Inf` rather than `NaN` from `log_sum_exp()` when both
  arguments are `-Inf`, matching Stan. This also affects
  [`nlf()`](https://paulbuerkner.com/brms/reference/brmsformula-helpers.md)
  formulas that call it. Thanks to Ahmed Eldeeb.
  ([\#1899](https://github.com/paul-buerkner/brms/issues/1899))
- Evaluate the cdf and survival function of `frechet`,
  `gen_extreme_value`, `discrete_weibull`, `asym_laplace`,
  `inverse.gaussian`, `exgaussian` and the zero-inflated and hurdle
  families on the log scale, so that truncated `log_lik` reaches them
  too. Thanks to Ahmed Eldeeb.
  ([\#1899](https://github.com/paul-buerkner/brms/issues/1899))
- Preserve the matrix structure of factors in `newdata`, which is
  required for matrix predictors of smooth terms such as `mrf`. Thanks
  to Sebastian Weber.
  ([\#1935](https://github.com/paul-buerkner/brms/issues/1935))
- Stop rounding `posterior_predict` output to integers for discrete
  models unless `output` is `"random"`, which collapsed probabilities
  and densities to 0 or 1. Thanks to Ahmed Eldeeb.
  ([\#1923](https://github.com/paul-buerkner/brms/issues/1923))
- Include the lower bound in `posterior_predict` outputs
  `"probability"`, `"pit"`, `"density"` and `"quantile"` for truncated
  discrete models, matching the other post-processing methods. Thanks to
  Ahmed Eldeeb.
  ([\#1923](https://github.com/paul-buerkner/brms/issues/1923))
...
- Normalize truncated `log_lik` by `P(lb <= Y <= ub)` for integer
  ...
  ([\#1903](https://github.com/paul-buerkner/brms/issues/1903))
...
- Align the R functions `log1m_exp()` and `log_diff_exp()` with their
  definitions in Stan.
  ([\#1899](https://github.com/paul-buerkner/brms/issues/1899))
- Cox models now place the knots of the baseline hazard based on the
  event times only instead of both event and censoring times. Models
  fitted with brms 2.23.1 or earlier keep using their original knots in
  post-processing. Thanks to Sebastian Weber.
  ([\#1898](https://github.com/paul-buerkner/brms/issues/1898))
- The basis of the baseline hazard of Cox models is now stored with the
  response instead of with the predictor terms. As a result, `log_lik`,
  `loo` and `kfold` applied to new data reuse the knots chosen at
  fitting time also for non-linear formulas. Thanks to Sebastian Weber.
  ([\#1898](https://github.com/paul-buerkner/brms/issues/1898))
- [`kfold_predict()`](https://paulbuerkner.com/brms/reference/kfold_predict.md)
  ...
  ([\#1889](https://github.com/paul-buerkner/brms/issues/1889))
- `bayes_R2` now uses model-based residual variances for Gaussian and
  Bernoulli models and falls back to residual-based computation for
  other families. This change may lead to changes in plotting results.
  ([\#1815](https://github.com/paul-buerkner/brms/issues/1815))
- Avoid the creation of zombie workers when executing `log_lik` in
  parallel thanks to Aki Vehtari and Noa Kallioinen. For now, `log_lik`
  will use PSOCK clusters if run in parallel even on Unix systems. To
  avoid potential speed loss for small models, `log_lik` will not use
  `option(mc.cores)` anymore. These changes may be reverted once the
  underlying causes of this issue have been fixed.
  ([\#1658](https://github.com/paul-buerkner/brms/issues/1658))
- Align the definition of the R function
  [`step()`](https://rdrr.io/r/stats/step.html) with the definition in
  Stan, such that `step(0) == 1` thanks to Daniel Sabanes Bov.
  ([\#1734](https://github.com/paul-buerkner/brms/issues/1734))
- Make
  [`read_csv_as_stanfit()`](https://paulbuerkner.com/brms/reference/read_csv_as_stanfit.md)
  store `adapt_delta` and `max_treedepth` values in `$control` so rstan
  can find these values. Thanks to Tristan Mahr
  ([\#1767](https://github.com/paul-buerkner/brms/issues/1767)).
- Enable updating argument `data2` for `brmsfit_multiple` objects.
  ([\#1776](https://github.com/paul-buerkner/brms/issues/1776))
- Restore the mode parameterization of the `com_poisson` family on the
  Stan side, which had switched to the classical parameterization in
  [\#1765](https://github.com/paul-buerkner/brms/issues/1765) while the
  R-side density functions did not, so that R-side post-processing no
  longer disagrees with the sampled density whenever `shape != 1`. Also
  correct the condition selecting the asymptotic normalizing constant,
  which was left unchanged by that switch and so no longer described the
  intended region. Models fitted with `com_poisson` and `shape != 1`
...
- Improve sampling efficiency of `beta_binomial` models.
  ([\#1703](https://github.com/paul-buerkner/brms/issues/1703))
- Improve efficiency of non-factorizable log-likelihoods for Student-t
...
- Support different Gaussian process kernels in `gp` terms.
  ([\#234](https://github.com/paul-buerkner/brms/issues/234))
- Support stratified `cox` models via the new addition term `bhaz`.
  ([\#1489](https://github.com/paul-buerkner/brms/issues/1489))
- Support futures for parallelization in the `cmdstanr` backend.
  ([\#1684](https://github.com/paul-buerkner/brms/issues/1684))
- Add method `loo_epred` thanks to Aki Vehtari.
  ([\#1641](https://github.com/paul-buerkner/brms/issues/1641))
- Add priorsense support via `create_priorsense_data.brmsfit` thanks to
  Noa Kallioinen.
  ([\#1354](https://github.com/paul-buerkner/brms/issues/1354))
- Vectorize censored log likelihoods in the Stan code when possible.
  ([\#1657](https://github.com/paul-buerkner/brms/issues/1657))
- Force Stan to activate threading without altering the Stan code via
  argument `force` of function `threading`.
  ([\#1549](https://github.com/paul-buerkner/brms/issues/1549))
- Support moment matching `loo` prediction methods.
  ([\#1674](https://github.com/paul-buerkner/brms/issues/1674))

#### Bug Fixes

- Fix a bug that led to partially duplicated Stan code in multilevel
  terms thanks to Henrik Singmann.
  ([\#1651](https://github.com/paul-buerkner/brms/issues/1651))
- Fix problems with parallel executions of post-processing functions
  ...
  Aki Vehtari, and Noa Kallioinen.
  ([\#1658](https://github.com/paul-buerkner/brms/issues/1658))
- Fix several minor bugs.
  ([\#1648](https://github.com/paul-buerkner/brms/issues/1648),
  [\#1644](https://github.com/paul-buerkner/brms/issues/1644),
  [\#1672](https://github.com/paul-buerkner/brms/issues/1672),
  [\#1642](https://github.com/paul-buerkner/brms/issues/1642),
  [\#1634](https://github.com/paul-buerkner/brms/issues/1634),
  [\#1666](https://github.com/paul-buerkner/brms/issues/1666),
  [\#1664](https://github.com/paul-buerkner/brms/issues/1664))

#### Other Changes

- Refactor some of the internal code base to avoid evaluating many
  data-dependent quantities several times.
  ([\#1653](https://github.com/paul-buerkner/brms/issues/1653))
- Smartly access internal functions when evaluating non-linear formulas.
  ([\#1635](https://github.com/paul-buerkner/brms/issues/1635))
- Improve the documentation in several places.
- Make argument `loo` optional in `loo_moment_match`.
- Change the output format of `loo_predict` and `loo_linpred` to be more
  consistent with other post-processing functions.
...
- Add experimental support for the `pathfinder` and `laplace` algorithms
  in the `cmdstanr` backend.
  ([\#1591](https://github.com/paul-buerkner/brms/issues/1591))
- Automatically recompute fit criteria previously stored in the model if
  potentially results-changing arguments are provided to the criterion
  method.
...
- Use several Stan built-in functions implemented since version 2.26 to
  improve the efficiency of multiple model classes.
  ([\#1077](https://github.com/paul-buerkner/brms/issues/1077))

#### Other Changes

- Change `make_stancode` and `make_standata` to be aliases of `stancode`
  and `standata`, respectively. Change `get_prior` to be an alias of a
  new generic method `default_prior`. This enable other packages to
  define new `stancode`, `standata` and `default_prior` methods to
  generate Stan code and data, and extract the default priors, for their
  own objects building on brms. Thanks to Ven Popov for helping with
  this. ([\#1604](https://github.com/paul-buerkner/brms/issues/1604))
- Change the default prior of the `shape` parameter of `negbinomial`
  models to `inv_gamma(0.4, 0.3)` thanks to Aki Vehtari.
  ([\#1614](https://github.com/paul-buerkner/brms/issues/1614))
- No longer automatically canonicalize the Stan code if cmdstanr is used
  as backend.
  ([\#1544](https://github.com/paul-buerkner/brms/issues/1544))
- Export `read_csv_as_stanfit` thanks to Ven Popov.
  ([\#1619](https://github.com/paul-buerkner/brms/issues/1619))
- Make installation of `shinystan` optional. This means that the package
  has to be loaded, via
  [`library(shinystan)`](https://mc-stan.org/shinystan/), before
  `launch_shinystan` can be used.
  ([\#1595](https://github.com/paul-buerkner/brms/issues/1595))
- Improve parameter class names in the `summary` output.
- Show histograms rather than densities in the `plot` method by default.
- Deprecate argument `N` in the `plot` method in favor of argument
  `nvariables`.
...
- Fix a bug in formula parsing of missing values terms with interactions
  thank to Guido Biele.
  ([\#1608](https://github.com/paul-buerkner/brms/issues/1608))
- Ensure compatibility of `combine_models` with moment matching.
  ([\#1603](https://github.com/paul-buerkner/brms/issues/1603))
- Ensure compatibility with the latest `splines2` package version.
  ([\#1580](https://github.com/paul-buerkner/brms/issues/1580))
- Fix output of `rmulti_normal` thanks to Ven Popov.
  ([\#1588](https://github.com/paul-buerkner/brms/issues/1588))
...
- Apply the `horseshoe` and `R2D2` priors globally, that is, for all
  additive predictor terms specified in the same formula.
  ([\#1492](https://github.com/paul-buerkner/brms/issues/1492))
- Use `as.brmsprior` to transform objects into a `brmsprior`.
  ([\#1491](https://github.com/paul-buerkner/brms/issues/1491))
- Use matrix data as non-linear covariates.
  ([\#1488](https://github.com/paul-buerkner/brms/issues/1488))
...
- No longer support multiple deprecated prior options for categorical
  ...
  ([\#1420](https://github.com/paul-buerkner/brms/issues/1420))
- Deprecate argument `newdata` of
  [`get_refmodel.brmsfit()`](https://paulbuerkner.com/brms/reference/get_refmodel.brmsfit.md).
  ([\#1502](https://github.com/paul-buerkner/brms/issues/1502))
- Disallow binomial models without `trials` argument after several years
  of deprecation.
  ([\#1501](https://github.com/paul-buerkner/brms/issues/1501))
...
- Fix a long-standing bug in the post-processing of spline models that
  ...
  different machine than where the model was originally fitted. Old
  ...
  Kalbitzer.
  ([\#1465](https://github.com/paul-buerkner/brms/issues/1465))
...
- Model unstructured autocorrelation matrices via the `unstr` term
  thanks to the help of Sebastian Weber.
  ([\#1435](https://github.com/paul-buerkner/brms/issues/1435))
- Model ordinal data with an extra category (non-response or similar)
  via the `hurdle_cumulative` family thanks to Stephen Wild.
  ([\#1448](https://github.com/paul-buerkner/brms/issues/1448))
...
- Add support for the latent projection available in **projpred**
  versions \>= 2.4.0.
  ([\#1451](https://github.com/paul-buerkner/brms/issues/1451))

#### Bug Fixes

- Fix a Stan syntax error in threaded models with `lasso` priors.
  ([\#1427](https://github.com/paul-buerkner/brms/issues/1427))
...
- Fix a bug for predictions in **projpred**, previously requiring more
  variables in `newdata` than necessary.
  ([\#1457](https://github.com/paul-buerkner/brms/issues/1457),
  [\#1459](https://github.com/paul-buerkner/brms/issues/1459),
  [\#1460](https://github.com/paul-buerkner/brms/issues/1460))
...
- Reuse user-specified control arguments originally passed to the Stan
  backend in `update` and related methods.
  ([\#1373](https://github.com/paul-buerkner/brms/issues/1373),
  [\#1378](https://github.com/paul-buerkner/brms/issues/1378))
- Allow to retain unused factors levels via `drop_unused_levels = FALSE`
  in `brm` and related functions.
  ([\#1346](https://github.com/paul-buerkner/brms/issues/1346))
- Automatically update old default priors based on new input when when
  updating models via `update.brmsfit`.
  ([\#1380](https://github.com/paul-buerkner/brms/issues/1380))
- Allow to use `dirichlet` priors for more parameter types.
  ([\#1165](https://github.com/paul-buerkner/brms/issues/1165))

#### Other Changes

- Improve efficiency of converting models fitted with
  `backend = "cmdstanr"` to `stanfit` objects thanks to Simon Mills and
  Jacob Socolar.
  ([\#1331](https://github.com/paul-buerkner/brms/issues/1331))
- Allow for more `O1` optimization of brms-generated Stan models thanks
  to Aki Vehtari.
  ([\#1382](https://github.com/paul-buerkner/brms/issues/1382))

#### Bug Fixes

- Fix problems with missing boundaries of `sdme` parameters in models
  with known response standard errors thanks to Solomon Kurz.
  ([\#1348](https://github.com/paul-buerkner/brms/issues/1348))
- Fix Stan code of `gamma` models with `softplus` link.
- Allow for more flexible data inputs to `brm_multiple`.
  ([\#1383](https://github.com/paul-buerkner/brms/issues/1383))
- Ensure that `control_params` returns the right values for models
  fitted with the `cmdstanr` backend.
  ([\#1390](https://github.com/paul-buerkner/brms/issues/1390))
- Fix problems in multivariate spline models when using the `subset`
  addition term.
  ([\#1385](https://github.com/paul-buerkner/brms/issues/1385))
...
- Add full user control for boundaries of most parameters via the `lb`
  and `ub` arguments of `set_prior` and related functions.
  ([\#878](https://github.com/paul-buerkner/brms/issues/878),
  [\#1094](https://github.com/paul-buerkner/brms/issues/1094))
- Add family `logistic_normal` for simplex responses.
  ([\#1274](https://github.com/paul-buerkner/brms/issues/1274))
...
- Add families `beta_binomial` & `zero_inflated_beta_binomial` for
  potentially over-dispersed and zero-inflated binomial response models
  thanks to Hayden Rabel.
  ([\#1319](https://github.com/paul-buerkner/brms/issues/1319) &
  [\#1311](https://github.com/paul-buerkner/brms/issues/1311))
- Display `ppd_*` plots in `pp_check` via argument `prefix`.
  ([\#1313](https://github.com/paul-buerkner/brms/issues/1313))
- Support the `log` link in binomial and beta type families.
  ([\#1316](https://github.com/paul-buerkner/brms/issues/1316))
- Support **projpred**’s augmented-data projection.
  ([\#1292](https://github.com/paul-buerkner/brms/issues/1292),
  [\#1294](https://github.com/paul-buerkner/brms/issues/1294))

#### Other changes

- Argument `brms_seed` has been added to
  [`get_refmodel.brmsfit()`](https://paulbuerkner.com/brms/reference/get_refmodel.brmsfit.md).
  ([\#1287](https://github.com/paul-buerkner/brms/issues/1287))
...
- Improve speed of the `summary` method for high-dimensional models.
  ([\#1330](https://github.com/paul-buerkner/brms/issues/1330))

#### Bug Fixes

- Fix Stan code of threaded multivariate models thanks to Anirban
  Mukherjee.
  ([\#1277](https://github.com/paul-buerkner/brms/issues/1277))
- Fix usage of `int_conditions` in `conditional_smooths` thanks to Urs
  Kalbitzer.
  ([\#1280](https://github.com/paul-buerkner/brms/issues/1280))
- Fix an error sometimes occurring for multilevel (reference) models in
  `projpred`’s K-fold CV.
  ([\#1286](https://github.com/paul-buerkner/brms/issues/1286))
- Fix response values in `make_standata` for `bernoulli` families when
  only 1s are present thanks to Facundo Munoz.
  ([\#1298](https://github.com/paul-buerkner/brms/issues/1298))
- Fix `pp_check` for censored responses to work for all plot types
  thanks to Hayden Rabel.
  ([\#1327](https://github.com/paul-buerkner/brms/issues/1327))
- Ensure that argument `overwrite` in `add_criterion` works as expected
  for all criteria thanks to Andrew Milne.
  ([\#1323](https://github.com/paul-buerkner/brms/issues/1323))
- Fix a problem in `launch_shinystan` occurring when warmup draws were
  saved thanks to Frank Weber.
  ([\#1257](https://github.com/paul-buerkner/brms/issues/1257),
  [\#1329](https://github.com/paul-buerkner/brms/issues/1329))
- Fix numerical stability problems in `log_lik` for ordinal models.
  ([\#1192](https://github.com/paul-buerkner/brms/issues/1192))
...
- Move `projpred` from `Imports:` to `Suggests:`. This has the important
  ...
  they want to use it (the more common case is probably attaching, which
  is achieved by [`library(projpred)`](https://mc-stan.org/projpred/)).
  ([\#1222](https://github.com/paul-buerkner/brms/issues/1222))

#### Bug Fixes

- Ensure that argument `overwrite` in `add_criterion` is working as
  intended thanks to Ruben Arslan.
  ([\#1219](https://github.com/paul-buerkner/brms/issues/1219))
- Fix a bug in
  [`get_refmodel.brmsfit()`](https://paulbuerkner.com/brms/reference/get_refmodel.brmsfit.md)
  (i.e., when using `projpred` for a `"brmsfit"`) causing offsets not to
  be recognized.
  ([\#1220](https://github.com/paul-buerkner/brms/issues/1220))
...
- Fix a bug causing problems during post-processing of models fitted
  ...
  Riccardo Fusaroli.
  ([\#1218](https://github.com/paul-buerkner/brms/issues/1218))
...
- Support several methods of the `posterior` package.
  ([\#1204](https://github.com/paul-buerkner/brms/issues/1204))
- Substantially extend compatibility of `brms` models with `emmeans`
  thanks to Mattan S. Ben-Shachar.
  ([\#907](https://github.com/paul-buerkner/brms/issues/907),
  [\#1134](https://github.com/paul-buerkner/brms/issues/1134))
- Combine missing value (`mi`) terms with `subset` addition terms.
  ([\#1063](https://github.com/paul-buerkner/brms/issues/1063))
- Expose function `get_dpar` for use in the post-processing of custom
  families thank to Martin Modrak.
  ([\#1131](https://github.com/paul-buerkner/brms/issues/1131))
...
- Add argument `incl_thres` to
  [`posterior_linpred.brmsfit()`](https://paulbuerkner.com/brms/reference/posterior_linpred.brmsfit.md)
  allowing to subtract the threshold-excluding linear predictor from the
  thresholds in case of an ordinal family.
  ([\#1137](https://github.com/paul-buerkner/brms/issues/1137))
- Add a `"mock"` backend option to facilitate testing thanks to Martin
  Modrak. ([\#1116](https://github.com/paul-buerkner/brms/issues/1116))
- Add option `file_refit = "always"` to always overwrite models stored
  via the `file` argument.
  ([\#1151](https://github.com/paul-buerkner/brms/issues/1151))
- Initial GPU support via OpenCL thanks to the help Rok Češnovar.
  ([\#1166](https://github.com/paul-buerkner/brms/issues/1166))
- Support argument `robust` in method `hypothesis`.
  ([\#1170](https://github.com/paul-buerkner/brms/issues/1170))
- Vectorize the Stan code of custom likelihoods via argument `loop` of
  `custom_family`.
  ([\#1084](https://github.com/paul-buerkner/brms/issues/1084))
- Experimentally allow category specific effects for ordinal
  `cumulative` models.
  ([\#1060](https://github.com/paul-buerkner/brms/issues/1060))
- Regenerate Stan code of an existing model via argument `regenerate` of
  method `stancode`.
- Support `expose_functions` for models fitted with the `cmdstanr`
  backend thanks to Sebastian Weber.
  ([\#1176](https://github.com/paul-buerkner/brms/issues/1176))
- Support `log_prob` and related functionality in models fitted with the
  `cmdstanr` backend via function `add_rstan_model`.
  ([\#1184](https://github.com/paul-buerkner/brms/issues/1184))
...
- Refactor and extend internal post-processing functions for ordinal and
  categorical models thanks to Frank Weber.
  ([\#1159](https://github.com/paul-buerkner/brms/issues/1159))
- Ignore `NA` values in interval censored boundaries as long as they are
  unused. ([\#1070](https://github.com/paul-buerkner/brms/issues/1070))
- Take offsets into account when deriving default priors for overall
  intercept parameters.
  ([\#923](https://github.com/paul-buerkner/brms/issues/923))
- Soft deprecate measurement error (`me`) terms in favor of the more
  general and consistent missing value (`mi`) terms.
  ([\#698](https://github.com/paul-buerkner/brms/issues/698))

#### Bug Fixes

- Fix an issue in the post-processing of non-normal ARMA models thanks
  to Thomas Buehrens.
  ([\#1149](https://github.com/paul-buerkner/brms/issues/1149))
- Fix an issue with default baseline hazard knots in `cox` models thanks
  to Malcolm Gillies.
  ([\#1143](https://github.com/paul-buerkner/brms/issues/1143))
- Fix a bug in non-linear models caused by accidental merging of
  ...
  ([\#1142](https://github.com/paul-buerkner/brms/issues/1142))
- Correctly trigger a refit for `file_refit = "on_change"` if factor
  level names have changed thanks to Martin Modrak.
  ([\#1128](https://github.com/paul-buerkner/brms/issues/1128))
- Validate factors in `validate_newdata` even when they are
  ...
  Martin Modrak.
  ([\#1141](https://github.com/paul-buerkner/brms/issues/1141))
- Fix a bug in the Stan code generation of threaded mixture models with
  predicted mixture probabilities thanks to Riccardo Fusaroli.
  ([\#1150](https://github.com/paul-buerkner/brms/issues/1150))
- Remove duplicated Stan code related to the `horseshoe` prior thanks to
  Max Joseph.
  ([\#1167](https://github.com/paul-buerkner/brms/issues/1167))
- Fix an issue in the post-processing of non-looped non-linear
  parameters thanks to Sebastian Weber.
- Fix an issue in the Stan code of threaded non-looped non-linear models
  thanks to Sebastian Weber.
  ([\#1175](https://github.com/paul-buerkner/brms/issues/1175))
...
- Turn off normalization in the Stan model via argument `normalize`. to
  increase sampling efficiency thanks to Andrew Johnson.
  ([\#1017](https://github.com/paul-buerkner/brms/issues/1017),
  [\#1053](https://github.com/paul-buerkner/brms/issues/1053))
...
- Refit models previously stored on disc only when necessary thanks to
  ...
  ([\#1058](https://github.com/paul-buerkner/brms/issues/1058))
- Allow for a finer tuning of informational messages printed in `brm`
  via the `silent` argument.
  ([\#1076](https://github.com/paul-buerkner/brms/issues/1076))
- Allow `stanvars` to alter distributional parameters.
  ([\#1061](https://github.com/paul-buerkner/brms/issues/1061))
- Allow `stanvars` to be used inside threaded likelihoods.
  ([\#1111](https://github.com/paul-buerkner/brms/issues/1111))

#### Other Changes

- Improve numerical stability of ordinal sequential models (families
  `sratio` and `cratio`) thanks to Andrew Johnson.
  ([\#1087](https://github.com/paul-buerkner/brms/issues/1087))

#### Bug Fixes

- Allow fitting `multinomial` models with the `cmdstanr` backend thanks
  to Andrew Johnson.
  ([\#1033](https://github.com/paul-buerkner/brms/issues/1033))
- Allow user-defined Stan functions in threaded models.
  ([\#1034](https://github.com/paul-buerkner/brms/issues/1034))
- Allow usage of the `:` operator in autocorrelation terms.
- Fix Stan code generation when specifying coefficient-level priors on
  spline terms.
- Fix numerical issues occurring in edge cases during post-processing of
  Gaussian processes thanks to Marta Kołczyńska.
- Fix an error during post-processing of new levels in multi-membership
  terms thanks to Guilherme Mohor.
- Fix a bug in the Stan code of threaded `wiener` drift diffusion models
  thanks to the GitHub user yanivabir.
  ([\#1085](https://github.com/paul-buerkner/brms/issues/1085))
- Fix a bug in the threaded Stan code for GPs with categorical `by`
  variables thanks to Reece Willoughby.
  ([\#1081](https://github.com/paul-buerkner/brms/issues/1081))
- Fix a bug in the threaded Stan code when using QR decomposition thanks
  to Steve Bronder.
  ([\#1086](https://github.com/paul-buerkner/brms/issues/1086))
- Include offsets in `emmeans` related methods thanks to Russell V.
  Lenth. ([\#1096](https://github.com/paul-buerkner/brms/issues/1096))
...
- Fix facet labels in `conditional_effects` thanks to Isaac Petersen.
  ([\#1014](https://github.com/paul-buerkner/brms/issues/1014))
...
- Experimentally support within-chain parallelization via `reduce_sum`
  using argument `threads` in `brm` thanks to Sebastian Weber.
  ([\#892](https://github.com/paul-buerkner/brms/issues/892))
- Add algorithm `fixed_param` to sample from fixed parameter values.
  ([\#973](https://github.com/paul-buerkner/brms/issues/973))
- No longer remove `NA` values in `data` if there are unused because of
  the `subset` addition argument.
  ([\#895](https://github.com/paul-buerkner/brms/issues/895))
- Combine `by` variables and within-group correlation matrices in
  group-level terms.
  ([\#674](https://github.com/paul-buerkner/brms/issues/674))
- Add argument `robust` to the `summary` method.
  ([\#976](https://github.com/paul-buerkner/brms/issues/976))
- Parallelize evaluation of the `posterior_predict` and `log_lik`
  methods via argument `cores`.
  ([\#819](https://github.com/paul-buerkner/brms/issues/819))
- Compute effective number of parameters in `kfold`.
- Show prior sources and vectorization in the `print` output of
  `brmsprior` objects.
  ([\#761](https://github.com/paul-buerkner/brms/issues/761))
...
- Support posterior mean predictions in `emmeans` via `dpar = "mean"`
  thanks to Russell V. Lenth.
  ([\#993](https://github.com/paul-buerkner/brms/issues/993))
- Improve control of which parameters should be saved via function
  `save_pars` and corresponding argument in `brm`.
  ([\#746](https://github.com/paul-buerkner/brms/issues/746))
- Add method `posterior_smooths` to computing predictions of individual
  smooth terms.
  ([\#738](https://github.com/paul-buerkner/brms/issues/738))
- Allow to display grouping variables in `conditional_effects` using the
  `effects` argument.
  ([\#1012](https://github.com/paul-buerkner/brms/issues/1012))

#### Other Changes

- Improve sampling efficiency for a lot of models by using Stan’s
  GLM-primitives even in non-GLM cases.
  ([\#984](https://github.com/paul-buerkner/brms/issues/984))
- Improve sampling efficiency of multilevel models with within-group
  covariances thanks to David Westergaard.
  ([\#977](https://github.com/paul-buerkner/brms/issues/977))
- Deprecate argument `probs` in the `conditional_effects` method in
  favor of argument `prob`.

#### Bug Fixes

- Fix a problem in `pp_check` inducing wronger observation orders in
  time series models thanks to Fiona Seaton.
  ([\#1007](https://github.com/paul-buerkner/brms/issues/1007))
...
- Support the Cox proportional hazards model for time-to-event data via
  family `cox`.
  ([\#230](https://github.com/paul-buerkner/brms/issues/230),
  [\#962](https://github.com/paul-buerkner/brms/issues/962))
...
- Improve the prediction behavior in post-processing methods when
  ...
  ([\#956](https://github.com/paul-buerkner/brms/issues/956))
...
- Fix shape parameters across multiple monotonic terms via argument `id`
  in function `mo` to ensure conditionally monotonic effects.
  ([\#924](https://github.com/paul-buerkner/brms/issues/924))
- Support package `rtdists` as additional backend of `wiener`
  distribution functions thanks to the help of Henrik Singmann.
  ([\#385](https://github.com/paul-buerkner/brms/issues/385))
...
- Fix generated Stan Code of models with improper global priors and
  ...
  ([\#919](https://github.com/paul-buerkner/brms/issues/919))
- Fix a bug in `conditional_effects` occurring for categorical models
  with matrix predictors thanks to Jamie Cranston.
  ([\#933](https://github.com/paul-buerkner/brms/issues/933))

#### Other Changes

- Adjust behavior of the `rate` addition term so that it also affects
  the `shape` parameter in `negbinomial` models thanks to Edward
  Abraham. ([\#915](https://github.com/paul-buerkner/brms/issues/915))
...
- Add arguments `cor`, `id`, and `cov` to the functions `gr` and `mm`
...
- Improve workflow to feed back brms-created models which were fitted
  somewhere else back into brms.
  ([\#745](https://github.com/paul-buerkner/brms/issues/745))
...
- Support multiple imputation of data passed via `data2` in
  `brm_multiple`.
  ([\#886](https://github.com/paul-buerkner/brms/issues/886))
- Fully support the `emmeans` package thanks to the help of Russell V.
  Lenth. ([\#418](https://github.com/paul-buerkner/brms/issues/418))
...
- Fix issue in Stan code of models with multiple `me` terms thanks to
  Chris Chatham.
  ([\#855](https://github.com/paul-buerkner/brms/issues/855),
  [\#856](https://github.com/paul-buerkner/brms/issues/856))
- Fix scaling problems in the estimation of ordinal models with multiple
  threshold vectors thanks to Marta Kołczyńska and Rok Češnovar.
- Allow usage of `std_normal` in `set_prior` thanks to Ben Goodrich.
  ([\#867](https://github.com/paul-buerkner/brms/issues/867))
- Fix Stan code of distributional models with `weibull`, `frechet`, or
  ...
  ([\#879](https://github.com/paul-buerkner/brms/issues/879))
- Fix Stan code of models which are truncated and weighted at the same
  time thanks to Michael Thompson.
  ([\#884](https://github.com/paul-buerkner/brms/issues/884))
- Fix Stan code of multivariate models with custom families and data
  variables passed to the likelihood thanks to Raoul Wolf.
  ([\#906](https://github.com/paul-buerkner/brms/issues/906))
...
- Rename `extract_draws` to `prepare_predictions` and deprecate the
  former function.
...
- Simplify the parameterization of the horseshoe prior thanks to Aki
  Vehtari. ([\#873](https://github.com/paul-buerkner/brms/issues/873))
...
- Fix parameters to constants via the `prior` argument.
  ([\#783](https://github.com/paul-buerkner/brms/issues/783))
- Specify autocorrelation terms directly in the model formula.
  ([\#708](https://github.com/paul-buerkner/brms/issues/708))
- Translate integer covariates in non-linear formulas to integer arrays
  in Stan.
- Estimate `sigma` in combination with fixed correlation matrices via
  autocorrelation term `fcor`.
- Use argument `data2` in `brm` and related functions to pass data
  objects which cannot be passed via `data`. The usage of `data2` will
  be extended in future versions.
- Compute pointwise log-likelihood values via `log_lik` for
  non-factorizable Student-t models.
  ([\#705](https://github.com/paul-buerkner/brms/issues/705))
...
- Fix selection of group-level terms via `re_formula` in multivariate
  models thanks to Maxime Dahirel.
  ([\#834](https://github.com/paul-buerkner/brms/issues/834))
- Enforce correct ordering of terms in `re_formula` thanks to
  [@ferberkl](https://github.com/ferberkl).
  ([\#844](https://github.com/paul-buerkner/brms/issues/844))
- Fix post-processing of multivariate multilevel models when multiple
  IDs are used for the same grouping factor thanks to
  [@lott999](https://github.com/lott999).
  ([\#835](https://github.com/paul-buerkner/brms/issues/835))
- Store response category names of ordinal models in the output of
  `posterior_predict` again thanks to Mattew Kay.
  ([\#838](https://github.com/paul-buerkner/brms/issues/838))
- Handle `NA` values more consistently in `posterior_table` thanks to
  Anna Hake. ([\#845](https://github.com/paul-buerkner/brms/issues/845))
...
- Rename `offset` variables to `offsets` in the generated Stan code as
  the former will be reserved in the new stanc3 compiler.
...
- Fix effective sample size note in the `summary` output.
  ([\#824](https://github.com/paul-buerkner/brms/issues/824))
- Fix an edge case in the handling of covariates in special terms thanks
  to Andrew Milne.
  ([\#823](https://github.com/paul-buerkner/brms/issues/823))
- Allow restructuring objects multiple times with different brms
  versions thanks to Jonathan A. Nations.
  ([\#828](https://github.com/paul-buerkner/brms/issues/828))
- Fix validation of ordered factors in `newdata` thanks to Andrew Milne.
  ([\#830](https://github.com/paul-buerkner/brms/issues/830))
...
- Fix problems when using `fitted(..., scale = "linear")` with
  multinomial models thanks to Santiago Olivella.
  ([\#770](https://github.com/paul-buerkner/brms/issues/770))
- Fix problems in the `as.mcmc` method for thinned models thanks to
  [@hoxo-m](https://github.com/hoxo-m).
  ([\#811](https://github.com/paul-buerkner/brms/issues/811))
- Fix problems in parsing covariates of special effects terms thanks to
  Riccardo Fusaroli
  ([\#813](https://github.com/paul-buerkner/brms/issues/813))

#### Other Changes

- Rename `marginal_effects` to `conditional_effects` and
  `marginal_smooths` to `conditional_smooths`.
  ([\#735](https://github.com/paul-buerkner/brms/issues/735))
- Rename `stanplot` to `mcmc_plot`.
- Add method `pp_expect` as an alias of `fitted`.
  ([\#644](https://github.com/paul-buerkner/brms/issues/644))
...
- Deprecate functions `add_loo` and `add_waic` in favor of
  `add_criterion`.
...
- Improve convergence diagnostics in the `summary` output.
  ([\#712](https://github.com/paul-buerkner/brms/issues/712))
- Use primitive Stan GLM functions whenever possible.
  ([\#703](https://github.com/paul-buerkner/brms/issues/703))
- Pass real and integer data vectors to custom families via the addition
  arguments `vreal` and `vint`.
  ([\#707](https://github.com/paul-buerkner/brms/issues/707))
- Model compound symmetry correlations via `cor_cosy`.
  ([\#403](https://github.com/paul-buerkner/brms/issues/403))
...
- Substantially improve the sampling efficiency of SAR models thanks to
  the GitHub user aslez.
  ([\#680](https://github.com/paul-buerkner/brms/issues/680))
...
- Set the number of trials to 1 by default in `marginal_effects` if not
  specified otherwise.
  ([\#718](https://github.com/paul-buerkner/brms/issues/718))
...
- Fix problems in the post-processing of `me` terms with grouping
  factors thanks to the GitHub user tatters.
  ([\#706](https://github.com/paul-buerkner/brms/issues/706))
- Allow grouping variables to start with a dot thanks to Bruno
  Nicenboim. ([\#679](https://github.com/paul-buerkner/brms/issues/679))
- Allow the `horseshoe` prior in categorical and related models thanks
  to the Github user tatters.
  ([\#678](https://github.com/paul-buerkner/brms/issues/678))
- Fix extraction of prior samples for overall intercepts in
  `prior_samples` thanks to Jonas Kristoffer Lindelov.
  ([\#696](https://github.com/paul-buerkner/brms/issues/696))
- Allow underscores to be used in category names of categorical
  responses thanks to Emmanuel Charpentier.
  ([\#672](https://github.com/paul-buerkner/brms/issues/672))
...
- Improve checks for non-standard variable names thanks to Ryan
  Holbrook. ([\#721](https://github.com/paul-buerkner/brms/issues/721))
- Fix problems when plotting facetted spaghetti plots via
  `marginal_smooths` thanks to Gavin Simpson.
  ([\#740](https://github.com/paul-buerkner/brms/issues/740))
...
- Specify non-linear ordinal models.
  ([\#623](https://github.com/paul-buerkner/brms/issues/623))
- Allow to fix thresholds in ordinal mixture models
  ([\#626](https://github.com/paul-buerkner/brms/issues/626))
- Use the `softplus` link function in various families.
  ([\#622](https://github.com/paul-buerkner/brms/issues/622))
- Use QR decomposition of design matrices via argument `decomp` of
  `brmsformula` thanks to the help of Ben Goodrich.
  ([\#640](https://github.com/paul-buerkner/brms/issues/640))
- Define argument `sparse` separately for each model formula.
- Allow using `bayes_R2` and `loo_R2` with ordinal models.
  ([\#639](https://github.com/paul-buerkner/brms/issues/639))
- Support `cor_arma` in non-normal models.
  ([\#648](https://github.com/paul-buerkner/brms/issues/648))

#### Other Changes

- Change the parameterization of monotonic effects to improve their
  interpretability.
  ([\#578](https://github.com/paul-buerkner/brms/issues/578))
...
- Fix a problem when applying `marginal_effects` to measurement error
  models thanks to Jonathan A. Nations.
  ([\#636](https://github.com/paul-buerkner/brms/issues/636))
...
- Allow compiling a model in `brm_multiple` without sampling thanks to
  Will Petry.
  ([\#671](https://github.com/paul-buerkner/brms/issues/671))
...
- Fit multinomial models via family `multinomial`.
  ([\#463](https://github.com/paul-buerkner/brms/issues/463))
- Fit Dirichlet models via family `dirichlet`.
  ([\#463](https://github.com/paul-buerkner/brms/issues/463))
- Fit conditional logistic models using the `categorical` and
  `multinomial` families together with non-linear formula syntax.
  ([\#560](https://github.com/paul-buerkner/brms/issues/560))
...
- Use different subsets of the data in different univariate parts of a
  multivariate model via addition argument `subset`.
  ([\#360](https://github.com/paul-buerkner/brms/issues/360))
- Control the centering of population-level design matrices via argument
  `center` of `brmsformula` and related functions.
- Add an `update` method for `brmsfit_multiple` objects.
  ([\#615](https://github.com/paul-buerkner/brms/issues/615))
- Split folds after `group` in the `kfold` method.
  ([\#619](https://github.com/paul-buerkner/brms/issues/619))
...
- Deprecate `compare_ic` and instead recommend `loo_compare` for the
  ...
  ([\#414](https://github.com/paul-buerkner/brms/issues/414))
- Use the **glue** package in the Stan code generation.
  ([\#549](https://github.com/paul-buerkner/brms/issues/549))
...
- Validate several sampling-related arguments in `brm` before compiling
  the Stan model.
  ([\#576](https://github.com/paul-buerkner/brms/issues/576))
- Show evaluated vignettes on CRAN again.
  ([\#591](https://github.com/paul-buerkner/brms/issues/591))
...
- Fix an error when trying to change argument `re_formula` in `bayes_R2`
  thanks to the GitHub user emieldl.
  ([\#592](https://github.com/paul-buerkner/brms/issues/592))
- Fix occasional problems when running chains in parallel via the
  **future** package thanks to Jared Knowles.
  ([\#579](https://github.com/paul-buerkner/brms/issues/579))
- Ensure correct ordering of response categories in ordinal models
  thanks to Jonas Kristoffer Lindelov.
  ([\#580](https://github.com/paul-buerkner/brms/issues/580))
- Ignore argument `resp` of `marginal_effects` in univariate models
  thanks to Vassilis Kehayas.
  ([\#589](https://github.com/paul-buerkner/brms/issues/589))
...
- Fix an error in the post-processing of monotonic effects occurring for
  multivariate models thanks to James Rae.
  ([\#598](https://github.com/paul-buerkner/brms/issues/598))
- Fix lower bounds in truncated discrete models.
- Fix checks of the original data in `kfold` thanks to the GitHub user
  gcolitti. ([\#602](https://github.com/paul-buerkner/brms/issues/602))
- Fix an error when applying the `VarCorr` method to meta-analytic
  models thanks to Michael Scharkow.
  ([\#616](https://github.com/paul-buerkner/brms/issues/616))
...
- Fit approximate and non-isotropic Gaussian processes via `gp`.
  ([\#540](https://github.com/paul-buerkner/brms/issues/540))
- Enable parallelization of model fitting in `brm_multiple` via the
  future package.
  ([\#364](https://github.com/paul-buerkner/brms/issues/364))
- Perform posterior predictions based on k-fold cross-validation via
  `kfold_predict`.
  ([\#468](https://github.com/paul-buerkner/brms/issues/468))
- Indicate observations for out-of-sample predictions in ARMA models via
  argument `oos` of `extract_draws`.
  ([\#539](https://github.com/paul-buerkner/brms/issues/539))

#### Other changes

- Allow factor-like variables in smooth terms.
  ([\#562](https://github.com/paul-buerkner/brms/issues/562))
...
- Fix an issue when that resulted in an error when using
  `fitted(..., scale = "linear")` with ordinal models thanks to Andrew
  Milne. ([\#557](https://github.com/paul-buerkner/brms/issues/557))
...
- Allow sampling from models with only a single observation that also
  contain an offset thanks to Antonio Vargas.
  ([\#545](https://github.com/paul-buerkner/brms/issues/545))
- Fix an error when sampling from priors in mixture models thanks to
  Jacki Buros Novik.
  ([\#542](https://github.com/paul-buerkner/brms/issues/542))
...
- Allow using `marginal_smooths` with ordinal models thanks to Andrew
  Milne. ([\#570](https://github.com/paul-buerkner/brms/issues/570))
- Fix an error in the post-processing of `me` terms thanks to the GitHub
  user hlluik.
  ([\#571](https://github.com/paul-buerkner/brms/issues/571))
...
- Specify separate priors for thresholds in ordinal models.
  ([\#524](https://github.com/paul-buerkner/brms/issues/524))
- Pass additional arguments to
  [`rstan::stan_model`](https://mc-stan.org/rstan/reference/stan_model.html)
  via argument `stan_model_args` in `brm`.
  ([\#525](https://github.com/paul-buerkner/brms/issues/525))
- Save model objects via argument `file` in `add_ic` after adding model
  fit criteria.
  ([\#478](https://github.com/paul-buerkner/brms/issues/478))
...
- Correctly plot splines with factorial covariates via
  `marginal_smooths`.
- Allow sampling from priors in intercept only models thanks to Emmanuel
  Charpentier.
  ([\#529](https://github.com/paul-buerkner/brms/issues/529))
...
- Improve `marginal_effects` to better display ordinal and categorical
  models via argument `categorical`.
  ([\#491](https://github.com/paul-buerkner/brms/issues/491),
  [\#497](https://github.com/paul-buerkner/brms/issues/497))
- Improve method `kfold` to offer more options for specifying omitted
  subsets. ([\#510](https://github.com/paul-buerkner/brms/issues/510))
...
- Allow using the `bridge_sampler` method even if prior samples are
  drawn within the model.
  ([\#485](https://github.com/paul-buerkner/brms/issues/485))
...
- Select a subset of coefficients in `fixef`, `ranef`, and `coef` via
  argument `pars`.
  ([\#520](https://github.com/paul-buerkner/brms/issues/520))
...
- Ignore argument `resp` when post-processing univariate models thanks
  to Ruben Arslan.
  ([\#488](https://github.com/paul-buerkner/brms/issues/488))
- Deprecate argument `ordinal` of `marginal_effects`.
  ([\#491](https://github.com/paul-buerkner/brms/issues/491))
- Deprecate argument `exact_loo` of `kfold`.
  ([\#510](https://github.com/paul-buerkner/brms/issues/510))
- Deprecate usage of `binomial` families without specifying `trials`.
- No longer sample from priors of population-level intercepts when using
  the default intercept parameterization.
...
- Remove stored fit indices when calling `update` on brmsfit objects
  ...
  ([\#490](https://github.com/paul-buerkner/brms/issues/490))
- Fix problems when predicting a single data point using spline models
  thanks to Emmanuel Charpentier.
  ([\#494](https://github.com/paul-buerkner/brms/issues/494))
- Set `Post.Prob = 1` if `Evid.Ratio = Inf` in method `hypothesis`
  thanks to Andrew Milne.
  ([\#509](https://github.com/paul-buerkner/brms/issues/509))
...
- Define custom variables in all of Stan’s program blocks via function
  `stanvar`. ([\#459](https://github.com/paul-buerkner/brms/issues/459))
- Change the scope of non-linear parameters to be global within
  univariate models.
  ([\#390](https://github.com/paul-buerkner/brms/issues/390))
- Allow to automatically group predictor values in Gaussian processes
  ...
  sampling efficiency.
  ([\#300](https://github.com/paul-buerkner/brms/issues/300))
...
- Fit non-linear mixture models.
  ([\#456](https://github.com/paul-buerkner/brms/issues/456))
- Fit censored or truncated mixture models.
  ([\#469](https://github.com/paul-buerkner/brms/issues/469))
...
- Conveniently save and load fitted model objects in `brm` via argument
  `file`. ([\#472](https://github.com/paul-buerkner/brms/issues/472))
...
- Allow custom families in mixture models thanks to Noam Ross.
  ([\#453](https://github.com/paul-buerkner/brms/issues/453))
- Ensure compatibility with **mice** version 3.0.
  ([\#455](https://github.com/paul-buerkner/brms/issues/455))
- Fix naming of correlation parameters of group-level terms with
  multiple subgroups thanks to Kristoffer Magnusson.
  ([\#457](https://github.com/paul-buerkner/brms/issues/457))
- Improve scaling of default priors in `lognormal` models
  ([\#460](https://github.com/paul-buerkner/brms/issues/460)).
...
- Allow censoring and truncation in zero-inflated and hurdle models.
  ([\#430](https://github.com/paul-buerkner/brms/issues/430))
...
- Improve sampling efficiency of the ordinal families `cumulative`,
  `sratio`, and `cratio`.
  ([\#433](https://github.com/paul-buerkner/brms/issues/433))
- Allow to specify a single k-fold subset in method `kfold`.
  ([\#441](https://github.com/paul-buerkner/brms/issues/441))

#### Bug fixes

- Fix a problem in `launch_shinystan` due to which the maximum treedepth
  was not correctly displayed thanks to Paul Galpern.
  ([\#431](https://github.com/paul-buerkner/brms/issues/431))
...
- Rename quantile columns in `posterior_summary`. This affects the
  output of `predict` and related methods if `summary = TRUE`.
  ([\#425](https://github.com/paul-buerkner/brms/issues/425))
...
- No longer set `pointwise` dynamically in `loo` and related methods.
  ([\#416](https://github.com/paul-buerkner/brms/issues/416))
- No longer show information criteria in the summary output.
- Simplify internal workflow to implement native response distributions.
  ([\#421](https://github.com/paul-buerkner/brms/issues/421))

#### Bug fixes

- Allow `cor_car` in multivariate models with residual correlations
  thanks to Quentin Read.
  ([\#427](https://github.com/paul-buerkner/brms/issues/427))
- Fix a problem in the Stan code generation of distributional `beta`
  models thanks to Hans van Calster.
  ([\#404](https://github.com/paul-buerkner/brms/issues/404))
- Fix `launch_shinystan.brmsfit` so that all parameters are now shown
  correctly in the diagnose tab.
  ([\#340](https://github.com/paul-buerkner/brms/issues/340))
...
- Specify custom response distributions with function `custom_family`.
  ([\#381](https://github.com/paul-buerkner/brms/issues/381))
- Model missing values and measurement error in responses using the `mi`
  addition term.
  ([\#27](https://github.com/paul-buerkner/brms/issues/27),
  [\#343](https://github.com/paul-buerkner/brms/issues/343))
- Allow missing values in predictors using `mi` terms on the right-hand
  side of model formulas.
  ([\#27](https://github.com/paul-buerkner/brms/issues/27))
- Model interactions between the special predictor terms `mo`, `me`, and
  `mi`. ([\#313](https://github.com/paul-buerkner/brms/issues/313))
- Introduce methods `model_weights` and `loo_model_weights` providing
  several options to compute model weights.
  ([\#268](https://github.com/paul-buerkner/brms/issues/268))
- Introduce method `posterior_average` to extract posterior samples
  averaged across models.
  ([\#386](https://github.com/paul-buerkner/brms/issues/386))
- Allow hyperparameters of group-level effects to vary over the levels
  of a categorical covariate using argument `by` in function `gr`.
  ([\#365](https://github.com/paul-buerkner/brms/issues/365))
- Allow predictions of measurement-error models with new data.
  ([\#335](https://github.com/paul-buerkner/brms/issues/335))
- Pass user-defined variables to Stan via `stanvar`.
  ([\#219](https://github.com/paul-buerkner/brms/issues/219),
  [\#357](https://github.com/paul-buerkner/brms/issues/357))
- Allow ordinal families in mixture models.
  ([\#389](https://github.com/paul-buerkner/brms/issues/389))
- Model covariates in multi-membership structures that vary over the
  levels of the grouping factor via `mmc` terms.
  ([\#353](https://github.com/paul-buerkner/brms/issues/353))
- Fit shifted log-normal models via family `shifted_lognormal`.
  ([\#218](https://github.com/paul-buerkner/brms/issues/218))
...
- Change the parameterization of `weibull` and `exgaussian` models to be
  consistent with other model classes. Post-processing of related models
  fitted with earlier version of `brms` is no longer possible.
...
- Improve output of the `hypothesis` method thanks to the ideas of Matti
  Vuorre. ([\#362](https://github.com/paul-buerkner/brms/issues/362))
...
- Allow the `:` operator to combine groups in multi-membership terms
  thanks to Gang Chen.
- Avoid an unexpected error when calling `LOO` with argument
  `reloo = TRUE` thanks to Peter Konings.
  ([\#348](https://github.com/paul-buerkner/brms/issues/348))
- Fix problems in `predict` when applied to categorical models thanks to
  Lydia Andreyevna Krasilnikova and Thomas Vladeck.
  ([\#336](https://github.com/paul-buerkner/brms/issues/336),
  [\#345](https://github.com/paul-buerkner/brms/issues/345))
- Allow truncation in multivariate models with missing values thanks to
  Malte Lau Petersen.
  ([\#380](https://github.com/paul-buerkner/brms/issues/380))
- Force time points to be unique within groups in autocorrelation
  structures thanks to Ruben Arslan.
  ([\#363](https://github.com/paul-buerkner/brms/issues/363))
- Fix problems when post-processing multiple uncorrelated group-level
  terms of the same grouping factor thanks to Ivy Jansen.
  ([\#374](https://github.com/paul-buerkner/brms/issues/374))
- Fix a problem in the Stan code of multivariate `weibull` and `frechet`
  models thanks to the GitHub user philj1s.
  ([\#375](https://github.com/paul-buerkner/brms/issues/375))
- Fix a rare error when post-processing `binomial` models thanks to the
  GitHub user SeanH94.
  ([\#382](https://github.com/paul-buerkner/brms/issues/382))
- Keep attributes of variables when preparing the `model.frame` thanks
  to Daniel Luedecke.
  ([\#393](https://github.com/paul-buerkner/brms/issues/393))
...
- Compute model averaged posterior predictions with method `pp_average`.
  ([\#319](https://github.com/paul-buerkner/brms/issues/319))
- Add new argument `ordinal` to `marginal_effects` to generate special
  ...
  silberzwiebel.
  ([\#190](https://github.com/paul-buerkner/brms/issues/190))
- Use informative inverse-gamma priors for length-scale parameters of
  Gaussian processes.
  ([\#275](https://github.com/paul-buerkner/brms/issues/275))
- Compute hypotheses for all levels of a grouping factor at once using
  argument `scope` in method `hypothesis`.
  ([\#327](https://github.com/paul-buerkner/brms/issues/327))
...
- Fix problems in renaming parameters of the `cor_bsts` structure thanks
  to Joshua Edward Morten.
  ([\#312](https://github.com/paul-buerkner/brms/issues/312))
- Fix some unexpected errors when predicting from ordinal models thanks
  to David Hervas and Florian Bader.
  ([\#306](https://github.com/paul-buerkner/brms/issues/306),
  [\#307](https://github.com/paul-buerkner/brms/issues/307),
  [\#331](https://github.com/paul-buerkner/brms/issues/331))
- Fix problems when estimating and predicting multivariate ordinal
  models thanks to David West.
  ([\#314](https://github.com/paul-buerkner/brms/issues/314))
- Fix various minor problems in autocorrelation structures thanks to
  David West.
  ([\#320](https://github.com/paul-buerkner/brms/issues/320))
...
- Fix incorrect computation of intercepts in `acat` and `cratio` models
  thanks to Peter Phalen.
  ([\#302](https://github.com/paul-buerkner/brms/issues/302))
...
to provide a clean new start for the package. Models fitted with `brms`
1.0 or higher should remain fully compatible with `brms` 2.0.
...
- Add support for generalized multivariate models, where each of the
  ...
  models are now also available in multivariate models.
  ([\#3](https://github.com/paul-buerkner/brms/issues/3))
...
- Optionally display the MC standard error in the `summary` output.
  ([\#280](https://github.com/paul-buerkner/brms/issues/280))
- Add argument `re.form` as an alias of `re_formula` to the methods
  ...
  consistency with other packages making use of these methods.
  ([\#283](https://github.com/paul-buerkner/brms/issues/283))
...
- Show the link functions of all distributional parameters in the
  `summary` output.
  ([\#277](https://github.com/paul-buerkner/brms/issues/277))
- Reduce working memory requirements when extracting posterior samples
  for use in `predict` and related methods thanks to Fanyi Zhang.
  ([\#224](https://github.com/paul-buerkner/brms/issues/224))
- Remove deprecated aliases of functions and arguments from the package.
  ([\#278](https://github.com/paul-buerkner/brms/issues/278))
- No longer support certain prior specifications, which were previously
  labeled as deprecated.
- Remove the deprecated addition term `disp` from the package.
- Remove old versions of methods `fixef`, `ranef`, `coef`, and
  `VarCorr`.
...
- Store `stan_funs` in `brmsfit` objects to allow using `update` on
  models with user-defined Stan functions thanks to Tom Wallis.
  ([\#288](https://github.com/paul-buerkner/brms/issues/288))
- Fix problems in various post-processing methods when applied to models
  with the reserved variable `intercept` in group-level terms thanks to
  the GitHub user ASKurz.
  ([\#279](https://github.com/paul-buerkner/brms/issues/279))
- Fix an unexpected error in `predict` and related methods when setting
  ...
  effect. Thanks to Timothy Mastny.
  ([\#286](https://github.com/paul-buerkner/brms/issues/286))
...
- Allow setting priors on noise-free variables specified via function
  `me`.
- Add arguments `Ksub`, `exact_loo` and `group` to method `kfold` for
  defining omitted subsets according to a grouping variable or factor.
- Allow addition argument `se` in `skew_normal` models.

#### Bug fixes

- Ensure correct behavior of horseshoe and lasso priors in multivariate
  models thanks to Donald Williams.
- Allow using `identity` links on all parameters of the `wiener` family
  thanks to Henrik Singmann.
  ([\#276](https://github.com/paul-buerkner/brms/issues/276))
- Use reasonable dimnames in the output of `fitted` when returning
  linear predictors of ordinal models thanks to the GitHub user atrolle.
  ([\#274](https://github.com/paul-buerkner/brms/issues/274))
...
- Rebuild monotonic effects from scratch to allow specifying
  ...
  ([\#239](https://github.com/paul-buerkner/brms/issues/239))
...
- Add argument `prob` to `summary`, which allows to control the width of
  the computed uncertainty intervals.
  ([\#259](https://github.com/paul-buerkner/brms/issues/259))
...
- Use the same noise-free variables for all model parts in measurement
  error models.
  ([\#257](https://github.com/paul-buerkner/brms/issues/257))
...
- No longer change the default theme of `ggplot2` when attaching `brms`.
  ([\#256](https://github.com/paul-buerkner/brms/issues/256))
- Make sure signs of estimates are not dropped when rounding to zero in
  `summary.brmsfit`.
  ([\#263](https://github.com/paul-buerkner/brms/issues/263))
...
- Perform model comparisons based on marginal likelihoods using the
  methods `bridge_sampler`, `bayes_factor`, and `post_prob` all powered
...
- Split the `nlpar` argument of `set_prior` into the three arguments
  `resp`, `dpar`, and `nlpar` to allow for more flexible prior
  specifications.
...
- Use the `launch_shinystan` generic provided by the `shinystan`
  package.
- Set
  [`bayesplot::theme_default()`](https://mc-stan.org/bayesplot/reference/theme_default.html)
  as the default `ggplot2` theme when attaching `brms`.
- Include citations of the `brms` overview paper as published in the
  Journal of Statistical Software.

#### Bug fixes

- Fix problems when calling `fitted` with `hurdle_lognormal` models
  thanks to Meghna Krishnadas.
- Fix problems when predicting `sigma` in `asym_laplace` models thanks
  to Anna Josefine Sorensen.
...
- Fit latent Gaussian processes of one or more covariates via function
  `gp` specified in the model formula
  ([\#221](https://github.com/paul-buerkner/brms/issues/221)).
- Rework methods `fixef`, `ranef`, `coef`, and `VarCorr` to be more
  flexible and consistent with other post-processing methods
  ([\#200](https://github.com/paul-buerkner/brms/issues/200)).
- Generalize method `hypothesis` to be applicable on all objects
  coercible to a `data.frame`
  ([\#198](https://github.com/paul-buerkner/brms/issues/198)).
...
- Introduce method `add_ic` to store and reuse information criteria in
  ...
  ([\#220](https://github.com/paul-buerkner/brms/issues/220)).
...
- Show output of code in HTML vignettes thanks to Ben Goodrich
  ([\#158](https://github.com/paul-buerkner/brms/issues/158)).
- Resolve citations in PDF vignettes thanks to Thomas Kluth
  ([\#223](https://github.com/paul-buerkner/brms/issues/223)).
- Improve sampling efficiency for `exgaussian` models thanks to Alex
  Forrence ([\#222](https://github.com/paul-buerkner/brms/issues/222)).
...
- Fix problems when setting priors on coefficients of auxiliary
  parameters when also setting priors on the corresponding coefficients
  of the mean parameter. Thanks to Matti Vuorre for reporting this bug.
- Allow ordered factors to be used as grouping variables thanks to the
  GitHub user itissid.

## brms 1.6.0

CRAN release: 2017-04-06
...
- Implement different ways to sample new levels of grouping factors in
  ...
  this feature.
- Add methods `loo_predict`, `loo_linpred`, and
  `loo_predictive_interval` for computing LOO predictions thanks to Aki
  Vehtari and Jonah Gabry.
...
- Introduce argument `negative_rt` in `predict` and `posterior_predict`
...
- Introduce argument `int_conditions` in `marginal_effects` for enhanced
  ...
  Kluth.
- Improve flexibility of the `conditions` argument of
  `marginal_effects`.
...
- Fix checks of argument `group` in method `pp_check` thanks to Thomas
  K.
...
- Import some generics from `nlme` instead of `lme4` to remove
  dependency on the latter one.
...
- Fix a bug leading to unexpected errors in some S3 methods when applied
  to ordinal models.
...
- Add the `lasso` prior as an alternative to the `horseshoe` prior for
  sparse models.
- Add the methods `log_posterior`, `nuts_params`, `rhat`, and
  `neff_ratio` for `brmsfit` objects to conveniently access quantities
  used to diagnose sampling behavior.
...
- Estimate the auxiliary parameter `sigma` in models with known standard
  errors of the response by setting argument `sigma` to `TRUE` in
  addition function `se`.
...
- Allow varying the scale of global shrinkage parameter of the
  `horseshoe` prior.
- Add functions `prior` and `prior_string` as aliases of `set_prior`,
  the former allowing to pass arguments without quotes `""` using
  non-standard evaluation.
- Introduce four new vignettes explaining how to fit non-linear models,
  distributional models, phylogenetic models, and monotonic effects
  respectively.
...
- Sample from the prior of the original population-level intercept when
  `sample_prior = TRUE` even in models with an internal temporary
  intercept used to improve sampling efficiency.
- Introduce methods `posterior_predict`, `predictive_error` and
  `log_lik` as (partial) aliases of `predict`, `residuals`, and `logLik`
  respectively.
...
- Avoid an error occurring during the prediction of new data when
  grouping factors with only a single factor level were supplied thanks
  to Tom Wallis.
...
- Allow to correctly `update` the `sample_prior` argument to value
  `"only"`.
- Fix an unexpected error occurring in many S3 methods when the thinning
  rate is not a divisor of the total number of posterior samples thanks
  to Paul Zerr.
...
- Fix a bug causing an error when passing lists to additional arguments
  of smoothing functions thanks to Wayne Folta.
- Fix a bug causing an error in the `prior_samples` method for models
  with multiple group-level terms that refer to the same grouping factor
  thanks to Marco Tullio Liuzza.
...
This is one of the largest updates of `brms` since its initial release.
In addition to many new features, the multivariate `'trait'` syntax has
been removed from the package as it was confusing for users, required
much special case coding, and was hard to maintain. See
...
- Zero-inflated and hurdle models do not use multivariate syntax anymore
  but instead have special auxiliary parameters named `zi` and `hu`
...
- Add support for generalized additive mixed models (GAMMs). Smoothing
  ...
  formula.
- Introduce `as.data.frame` and `as.matrix` methods for `brmsfit`
  objects.
...
- Allow to compute `WAIC` and `LOO` based on the pointwise
  log-likelihood using argument `pointwise` to substantially reduce
  memory requirements.
...
- Fix a bug that could lead to an error when calling `marginal_effects`
  for predictors that were generated with the
  [`base::scale`](https://rdrr.io/r/base/scale.html) function thanks to
  Tom Wallis.
- Allow interactions of numeric and categorical predictors in
  `marginal_effects` to be passed to the `effects` argument in any
  order.
...
- Allow to pass self-defined `Stan` functions via argument `stan_funs`
  of `brm`.
- Add the `expose_functions` method allowing to expose self-defined
  `Stan` functions in `R`.
...
- Fix a bug that prohibited to change priors on single standard
  ...
  Charpentier.
- Fix a bug that prohibited to use nested grouping factors in non-linear
  models thanks to Tom Wallis.
- Fix a bug in the linear predictor computation within `R`, occurring
  for ordinal models with multiple category specific effects. This could
  lead to incorrect outputs of `predict`, `fitted`, and `logLik` for
  these models.
...
- The `cauchy` family is now deprecated and will be removed soon as it
  ...
  anyway.
- Change the default settings of the number of chains and warmup samples
  to the defaults of `rstan` (i.e., `chains = 4` and
  `warmup = iter / 2`).
...
- Move code of self-defined `Stan` functions to `inst/chunks` and
  incorporate them into the models using
  [`rstan::stanc_builder`](https://mc-stan.org/rstan/reference/stanc.html).
...
- Fix some backwards compatibility issues of models fitted with version
  \<= 0.5.0 thanks to Ulf Koether.
...
- Generalize the `formula` argument for zero-inflated and hurdle models
  so that predictors can be included in only one of the two model parts
  thanks to the idea of Wade Blanchard.
...
- Use some generic functions of the `lme4` package to avoid unnecessary
  ...
  method `VarCorr`.
- Change the `ggplot` theme in the `plot` method through argument
  `theme`.
- Remove the `n.` prefix in arguments `n.iter`, `n.warmup`, `n.thin`,
  `n.chains`, and `n.cluster` of the `brm` function. The old argument
  names remain usable as deprecated aliases.
...
- Add support for zero-inflated and hurdle models thanks to the idea of
  Scott Baldwin.
- Implement inverse gaussian models through family `inverse.gaussian`.
- Allow to specify truncation boundaries of the response variable thanks
  to the idea of Maciej Beresewicz.
- Add support for autoregressive (AR) effects of residuals, which can be
  modeled using the `cor_ar` and `cor_arma` functions.
...
- Move autoregressive (AR) effects of the response from the `cor_ar` to
  ...
  residuals.
- Improve checks on argument `newdata` used in the `fitted` and
  `predict` method.
...
- Improve documentation to better explain differences in autoregressive
  effects across R packages.
...
- Fix a bug that could cause an error for models with multiple grouping
  factors thanks to Jonathan Williams.
...
- Introduce new family `bernoulli` for dichotomous response variables as
  a more efficient alternative to families `binomial` or `categorical`
...
- Slightly change the internal structure of brms to reflect that `rstan`
  is finally on CRAN.
- Thoroughly check validity of the response variable before the data is
  passed to `Stan`.
...
- Always center random effects estimates returned by S3 method `ranef`
  around zero.
- Prevent the use of customized covariance matrices for grouping factors
  with multiple random effects for now.
...
- Fix a bug that could cause the fixed intercept to be wrongly estimated
  in the presence of multiple random intercepts thanks to Jarrod
  Hadfield.
...
- Introduce new argument `addition` in function brm to get a more
  flexible approach in specifying additional information on the response
  variable (e.g., standard errors for meta-analysis). Alternatively,
  this information can also be passed to the `formula` argument
  directly.
...
- Introduce new argument `cov.ranef` in the `brm` function allowing for
  customized covariance structures of random effects thanks to the idea
  of Boby Mathew.
...
- Amend parametrization of random effects to increase efficiency of the
  sampling algorithms.
...
- New methods for class `brmsfit`: `summary`, `print`, `plot`,
  `predict`, `fixef`, `ranef`, `VarCorr`, `nobs`, `ngrps`, and
  `formula`.
- Introduce new argument `silent` in the `brm` function, allowing to
  suppress most of `Stan`’s intermediate output.
- Introduce new families `negbinomial` (negative binomial) and
  `geometric` to allow for more flexibility in modeling count data.
...
- Fix a bug that caused an error when posterior predictives were sampled
  for family `cumulative`.
- Fix a bug that prohibited to use of improper flat priors for
  parameters that have proper priors by default.

## brms 0.1.0

CRAN release: 2015-05-08
